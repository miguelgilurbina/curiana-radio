...
reporta notas de `mocs/` o `investigacion/fuentes/` sin ningún enlace entrante.
...
Uso:
    python investigacion/check_vault_links.py          # informe
    python investigacion/check_vault_links.py --strict # exit 1 si hay roto
...
def indexar():
    ...
    for root, f in _caminar():
        ...
        if ext.lower() in EXT_ENLAZABLES:
            idx[stem].append(os.path.relpath(os.path.join(root, f), VAULT))
    return idx
...
def main():
    ...
    vigilados = (os.path.join("mocs", ""), os.path.join("investigacion", "fuentes", ""))
    huerfanas = sorted(
        b for b, rutas in idx.items()
        if b not in entrantes
        and any(r.endswith(".md") and r.startswith(vigilados) for r in rutas)
    )
    if huerfanas:
