...
MAX_TOKENS_AGENT = 450      # Un poco más para dar espacio a neologismos entre []
...
def call_agent(
    ...
    system_parts = [base_prompt, "---", world_context, f"[Tu ubicación]: {ubicacion}"]
    if bloque_lexico:
...
def interactive_mode(client: anthropic.Anthropic):
    ...
    while True:
        ...
        if not user_input:
            run_turn(client, state, memory, lexico, observer)
            ...
            run_turn(client, state, memory, lexico, observer, user_input=user_input)
...
def auto_mode(
    ...
    for t in range(turnos):
        run_turn(client, state, memory, lexico, observer, verbose=verbose)
