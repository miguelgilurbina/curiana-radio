...
VOCABULARIO_BASE: dict[str, dict] = {
    ...
    "curiana":    {"sig": "territorio de los caquetíos / lugar del cardón", "cat": "topón", "fuente": "caquetío-atestiguado", "notas": "Zavala Reyes 2015, nota al pie (4): «Curiana: territorio donde estaban asentados los caquetíos» · REPARTO 2026-09-10: a ATESTIGUADO. Zavala nota al pie (4) la glosa como topónimo-etnónimo, y el canon la sostiene por todos lados — es el nombre del proyecto y el del pueblo que Arcaya identifica con Coro.", },

    # ── Arahuaco compartido (cognados en Wayunaiki, Lokono, Taíno) ──
    "wayuu":      {"sig": "persona, gente, ser humano",                     "cat": "sust",  "fuente": "wayunaiki"},
    "saika":        {"sig": "bueno, bien, bello", "cat": "v_estativo", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. achagua «Saica» (Neira y Ribero 1762, pliegos 43d-44i: «Saicauní, Bueno está», «Saicay, Buena casa»); en el lokono la segmentación de Perea aísla SA en la palabra que traduce BIEN (Perea Alonso 1942 p. 499, «Ú-SA-NÚ-wai … has hecho BIEN»). Dos hermanas en la raíz, la forma de una. Estativo: el estado es verbo (d21.4). Menos de dos hermanas con la misma forma (N1 → A, th.2): hipotética; el perfil era2 no la muestrea y la enseña la plantilla breve (línea VOCES). Sustituye a `anasa` ('bueno, bien, bello (< anasü Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "dai": {"sig": "yo (1ra persona singular)", "cat": "pron", "fuente": "caquetío-reconstruido", "notas": "D11 fase 3 (tf.1, 2026-09-23, «Los pronombres, ok»): las formas reconstruidas desde el wayuu salen del habla; ésta entra desde las hermanas que Miguel nombró (cc.12: lokono, taíno, achagua). Propuesta: 6-fusion/propuesta_d11_fase3_pronombres_aspectos_2026-09-23.yaml. Lokono «Sg. 1 cm. da- i , da-kia = yo» (Perea Alonso 1942 p. 573; igual en los tres moravos, p. 581); el taíno comparte el prefijo `da-` (`daca` 'yo', Pané cap. XXV con la glosa de Las Casas, Apologética p. 447). C1 de Oliver 1989 p. 136 (*/nV-/ > /dA-/ en lokono y taíno, «and perhaps Caquetío»; traza caquetía en `dare`). Reconstruido por «dos de las tres hermanas» (tf.1). La base -i es la lokona: por C9 las vocales no se derivan. Sustituye a `taya`, archivada · D1 de la tanda de las hermanas (2026-09-24): el habla de mujeres kalinago NO apoya esta forma — dice n(u)-, noukoya 'yo' (Goeje 1939 p. 24; Adam 1879 p. 278), como el achagua nu-. Dos hermanas contra dos: `dai` se queda por lokono + taíno (tf.1). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "bui": {"sig": "tú (2da persona singular)", "cat": "pron", "fuente": "caquetío-reconstruido", "notas": "D11 fase 3 (tf.1, 2026-09-23, «Los pronombres, ok»): las formas reconstruidas desde el wayuu salen del habla; ésta entra desde las hermanas que Miguel nombró (cc.12: lokono, taíno, achagua). Propuesta: 6-fusion/propuesta_d11_fase3_pronombres_aspectos_2026-09-23.yaml. Lokono «bu-i, bo-kia = tú» (Perea Alonso 1942 p. 573; p. 581). La *b* por C3 (Oliver 1989 pp. 119, 150: el caquetío tiene la *b* lokona donde el wayuu tiene *p*). Una sola hermana: hipotético. Sustituye a `pia`, archivada. El trato formal sigue siendo `kudanga` (d21.10) · D1 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): el habla de MUJERES kalinago cuenta como hermana (es la capa arahuaca: Adam 1879 pp. 279-280) y dice la misma forma que el lokono — lokono bu-, kalinago b(u)-, boukoya 'tú' (Goeje 1939 p. 24; Adam 1879 pp. 277-279). Dos hermanas con la misma forma: pasa a RECONSTRUIDO (N1 → A). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml §decision_d1 (era `caquetío-hipotético`)"},
    "lihi": {"sig": "él (3ra persona singular, varón)", "cat": "pron", "fuente": "caquetío-reconstruido", "notas": "D11 fase 3 (tf.1, 2026-09-23, «Los pronombres, ok»): las formas reconstruidas desde el wayuu salen del habla; ésta entra desde las hermanas que Miguel nombró (cc.12: lokono, taíno, achagua). Propuesta: 6-fusion/propuesta_d11_fase3_pronombres_aspectos_2026-09-23.yaml. Lokono «li-hi» varonil (Perea Alonso 1942 p. 573). Las tres hermanas distinguen el género en el pronombre (lokono, achagua «Ria»/«Ruya», kalinago): ninguna tiene un 3sg sin género, así que un pronombre único sería convención nuestra. Reabre d21.13 SÓLO en el pronombre: el género de los nombres sigue sin importarse. Con `tuhu`, sustituye a `nüma`, archivada · D1 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): el habla de MUJERES kalinago cuenta como hermana (es la capa arahuaca: Adam 1879 pp. 279-280) y dice la misma forma que el lokono — lokono li-hi, kalinago l(i)- (común), likia 'él' (Goeje 1939 p. 24; Adam 1879 pp. 277-279). Dos hermanas con la misma forma: pasa a RECONSTRUIDO (N1 → A). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml §decision_d1 (era `caquetío-hipotético`)"},
    "tuhu": {"sig": "ella, ello (3ra persona singular, no varón: mujer, animal, cosa)", "cat": "pron", "fuente": "caquetío-reconstruido", "notas": "D11 fase 3 (tf.1, 2026-09-23, «Los pronombres, ok»): las formas reconstruidas desde el wayuu salen del habla; ésta entra desde las hermanas que Miguel nombró (cc.12: lokono, taíno, achagua). Propuesta: 6-fusion/propuesta_d11_fase3_pronombres_aspectos_2026-09-23.yaml. Lokono «tu-hu» no varonil (Perea Alonso 1942 p. 573), que en lokono es la clase amplia (p. 554): mujer, animal, cosa. La clave lokono `tuhu` 'aquel…' pasa a `tuhu-lokono`. Con `lihi`, sustituye a `nüma`, archivada · D1 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): el habla de MUJERES kalinago cuenta como hermana (es la capa arahuaca: Adam 1879 pp. 279-280) y dice la misma forma que el lokono — lokono tu-hu, kalinago t(u)- (común), tokoya 'ella' (Goeje 1939 p. 24; Adam 1879 pp. 277-279). Dos hermanas con la misma forma: pasa a RECONSTRUIDO (N1 → A). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml §decision_d1 (era `caquetío-hipotético`)"},
    ...
    "diki": {"sig": "ver, mirar, observar", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "D11 fase 3, las voces que quedaban (tf.5, 2026-09-23: «Una gente más le busca las sustitutas […] Sí, me parece eso»). Lokono `a-ddiki-n` 'ver, mirar' (Perea Alonso 1942 pp. 479 y 389), la misma raíz en los cuatro estratos (Schultz, Brinton, Oliver A-2, Pet), en grafía fonémica sin la `dd` de los moravos. Una sola hermana (el achagua da `nu-caba-u`). Sustituye a `wana`. Propuesta: 6-fusion/propuesta_d11_voces_wayuu_2026-09-23.yaml · N1 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): gana la segunda hermana — mujeres kalinago «arika», A adika (Goeje 1939 p. 100, visto en imagen; lokono d = kalinago r, Adam 1879 pp. 288-289): pasa a RECONSTRUIDO. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-hipotético`)"},
    ...
    "kunu":         {"sig": "ir, moverse hacia", "cat": "v_raiz", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono cunnu 'ir' (Perea Alonso 1942 p. 623, vocabulario de Schumann) y A-CUN-TE (Fraseario p. 362, cuatro atestaciones), sin la geminada. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `naa` ('ir, moverse hacia'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "sile":         {"sig": "venir, aproximarse", "cat": "v_raiz", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. mujeres kalinago «venir, arriver, advenir f šile» (Goeje 1939 p. 105, visto en imagen, sigla f am: de origen arahuaco). Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `waa` ('venir, aproximarse'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "kaa": {"sig": "estar, existir, ser (cópula)", "cat": "v_raiz", "notas": "núcleo fundacional, forma justificada por cognado en proto-arahuaco · Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 4: se queda y se DEGRADA: ninguna hermana da una cópula que se parezca, y cambiarla tocaría toda la gramática enseñada. Pasa a HIPOTÉTICO (N1 → A: no llega a dos hermanas); el perfil era2 no la muestrea y llega por plantilla. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-reconstruido`)", "fuente": "caquetío-hipotético"},
    ...
    "maa": {"sig": "decir, hablar, comunicar", "cat": "v_raiz", "notas": "núcleo fundacional, forma justificada por cognado en proto-arahuaco · Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 4: se queda con cita: achagua «Numau, Decir» = nu-ma-u (Neira y Ribero 1762, pliego 55 der.), una hermana. Pasa a HIPOTÉTICO (N1 → A: no llega a dos hermanas); el perfil era2 no la muestrea y llega por plantilla. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-reconstruido`)", "fuente": "caquetío-hipotético"},
    "butu":         {"sig": "tomar, coger, recibir", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. Goeje 1939 p. 74 (visto en imagen, sigla fhA): «cueillir (a)bula-ka, abutu … travailler abutu, cueillir … A abuti … prendre A abutu» — el kalinago (los dos registros) y el lokono, la misma raíz; sin el prefijo a- del infinitivo, como `diki`. Hallada en la transcripción del habla de mujeres del 2026-09-24. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `taa` ('tomar, coger, recibir'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "ani":          {"sig": "hacer, construir", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. Goeje 1939 p. 94 (visto en imagen): «faire h ani, aniro, anikua, A ani» — lokono y kalinago, la misma forma (en el kalinago es la de los hombres y la común, sigla 1 am 139, de origen arahuaco; las mujeres dicen ateka, p. 109); Perea Alonso 1942 A-NI-SIA 'lo hecho' (p. 346). Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `chaa` ('hacer, construir'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "dunku":        {"sig": "dormir, reposar, descansar", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. Goeje 1939 p. 101 (visto en imagen): «dormir f aromanka … A adunku»; Adam 1879 p. 288: lokono «Adum-ki-n» → mujeres «aroman-ca». Lokono d = kalinago r (Adam pp. 288-289). La forma lokona sin el a- del infinitivo. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `suna` ('dormir, reposar, descansar'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "aeke":         {"sig": "comer, alimentarse", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. lokono «Aeke, manger» → mujeres kalinago «aica» (Adam 1879 p. 288, visto en imagen); Goeje 1939 p. 78 (imagen): «manger f aika … A aeke, eke». La forma lokona. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `masa` ('comer, alimentarse'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "ati":          {"sig": "beber, tomar líquido", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. lokono «Atti-n, boire» → mujeres kalinago «ata-ca» (Adam 1879 p. 289, visto en imagen); Perea Alonso 1942 M-A-TTI-N 'no bebió' (p. 250); Goeje 1939 p. 107 (imagen): «boire f ata … A ata, ato». La raíz lokona, sin la geminada; `ata` tiene vecinas en el lexicón y no se usa. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `awa` ('beber, tomar líquido'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "aita":         {"sig": "saber, conocer, entender", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. Goeje 1939 p. 101 (visto en imagen): «se mémorer aritaga … A aitoa, connaître A adita, aita»: lokono aita, kalinago aritaga (d = r). Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `panaa` ('saber, conocer, entender'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "hiti":         {"sig": "querer, desear, necesitar", "cat": "v_raiz", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. Goeje 1939 p. 110 (visto en imagen): «désirer A hiti, hite»; p. 109 «vouloir, disposé à A ahite(-ka)»; Perea Alonso 1942 -HITTI desiderativo (L-A-KI-N-HITTI-CU-MA 'quiso comer', p. 258). Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `raka` ('querer, desear, necesitar'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "kudu":         {"sig": "cargar, transportar, llevar", "cat": "v_raiz", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono KÙDDÙ 'carga' (Perea Alonso 1942 p. 253) y cudu-cutta 'llevar' (p. 378); Goeje 1939 p. 98 «pesant A kudu». Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `rua` ('cargar, transportar, llevar'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},

    # ── Naturaleza (cognados arahuacos) ─────────────────────────────
    "uni":          {"sig": "agua (corriente, bebible)", "cat": "sust", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. achagua «Vni, Agua» (Neira y Ribero 1762, pliego 32 izq.; V inicial = u) y lokono wuini / oni-abo: la raíz arahuaca *uni en dos de las tres hermanas que Miguel nombró. La clave achagua `uni` pasa a `uni-achagua`. `duna` era además el tuna kalina, caribe de hombres (inventario del 2026-09-23). Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `duna` ('agua (corriente, bebible)'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "hikihi":       {"sig": "fuego, lumbre, brasa", "cat": "sust", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono HIKKI-HI 'fuego' (Perea Alonso 1942 p. 46), la forma entera: `hiki` quedaba a una letra de `diki` 'ver'. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `amana` ('fuego, lumbre, brasa'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "unia":         {"sig": "lluvia, agua del cielo", "cat": "sust", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. achagua «Vnia, Lluvia» (Neira y Ribero 1762, pliego 73 der.) y lokono oni 'lluvia': la misma raíz del agua. La clave achagua `unia` pasa a `unia-achagua`. ⚠️ A una letra de `uria` 'plantío'. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `kaya` ('lluvia, agua del cielo'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "ada":          {"sig": "árbol, madera, tronco", "cat": "sust", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. lokono «Ada, bois, arbre, forêt» → kalinago «ara-bou, dans le bois» (Adam 1879 p. 288, visto en imagen): d = r; y el lokono ada de la Tabla A-2 de Oliver (fila 23 'tree'), cuya clave pasa a COLISIONES_A2. Mantiene la CONVIVENCIA DECLARADA de d21.16: `ada` es el árbol vivo, `bara` el palo. Pista, no atestación: el topónimo caquetío «Adabacoa» 'todo arboleda' (lexicon_zavala.TOPONIMOS_ZAVALA) — una etimología de topónimo no atestigua (tf.0, 3-a). Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `kuru` ('árbol, madera, tronco'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "hime":         {"sig": "pez, pescado", "cat": "sust", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono hime (Oliver 1989, Apéndice A, Tabla A-2, fila 19 'fish') e itime (Brinton). La clave lokono de la A-2 pasa a COLISIONES_A2. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `arima` ('pez, pescado'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "wunabu":       {"sig": "tierra, suelo, polvo", "cat": "sust", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono WUNABU 'tierra' (Perea Alonso 1942 pp. 95, 70, 96; siete atestaciones). La clave lokono pasa a `wunabu-lokono`. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `dali` ('tierra, suelo, polvo'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "suka": {"sig": "noche, oscuridad", "cat": "sust", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 4: se queda con cita: lokono CA-SACCU-DA 'de noche' (Perea Alonso 1942 p. 71), una hermana. Pasa a HIPOTÉTICO (N1 → A: no llega a dos hermanas); el perfil era2 no la muestrea y llega por plantilla. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-reconstruido`)", "fuente": "caquetío-hipotético"},
    "bara":       {"sig": "palo, árbol", "cat": "sust", "notas": "Decisión #101 (tanda 2026-08-30) — GLOSA CORREGIDA a la de las fuentes: Zavala Reyes 2015 #29 (E): «Palo, árbol», con Esteves 1989 y van Buurt 2014 diciendo lo mismo, y la prueba interna de barabara (Zavala Reyes 2015 #30: «Árbol de madera dura y pesada. Olivo») — la reduplicada de la misma raíz ya estaba atestiguada como árbol. Lectura descartada por D7: río, corriente fluvial (venía de cognado proto-arawakan/topónimo, sin cita) — queda registrada aquí, no se pierde · d21.16 (2026-09-21), punto 4 — CONVIVENCIA DECLARADA con `kuru` 'árbol, madera, tronco' (reconstruida). No es un bug de curación ni un par de la política «manda la atestiguada»: es variación real y las dos se quedan. `bara` es el PALO CORTADO —y `barabara`, su reduplicada atestiguada, la madera dura (Zavala #30)—; `kuru`, el árbol vivo · CAPA CAMBIADA 2026-09-23 (cc.4 / tf.0, opción B: la sigla (E) de Zavala es Esteves 1989): era `caquetío-atestiguado`, pasa a `caquetío-reconstruido`. Zavala #29 (E) es Esteves, y Esteves lo afirma sin decir de dónde —Barabara (p. 19, OCR): «Sabemos que Bara, en lengua caquetía, es voz general para… árbol»; lo repite en Baraived (p. 22) y Bariquí (p. 88)—. «Con Esteves 1989 y van Buurt 2014 diciendo lo mismo» cuenta dos veces a Esteves: van Buurt §5 («bara means tree in Caquetío») va en el párrafo que cita a Esteves. Lo que la sube de hipotética a reconstruida es que corrobora una forma de otra fuente: BARABARA, «Árbol de Coro», Alvarado 1921 p. 21, reduplicada y sin partir (la partición es de Esteves); el lokono balli 'árbol' la acerca. La convivencia con `kuru` (d21.16) no cambia: su motivo era la sinonimia, no la capa. Medido en 6-fusion/medicion_sigla_E_zavala_2026-09-23.yaml · Tanda de las hermanas (2026-09-24): `kuru` se archivó y el árbol vivo es ahora `ada` (lokono ada = kalinago ara-, Adam 1879 p. 288); la CONVIVENCIA DECLARADA de d21.16 sigue, con `ada` en el lugar de `kuru`", "fuente": "caquetío-reconstruido", "forma_fuente": "Bara"},

    # ── Personas y parentesco ──────────────────────────────────────────
    "ama": {"sig": "madre, mujer que nutre y da origen", "cat": "sust", "notas": "núcleo fundacional, forma justificada por cognado en proto-arawakan universal · Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 4: se queda y se degrada: voz infantil que casi toda lengua tiene; no es evidencia de parentesco. Pasa a HIPOTÉTICO (N1 → A: no llega a dos hermanas); el perfil era2 no la muestrea y llega por plantilla. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-reconstruido`)", "fuente": "caquetío-hipotético"},
    "iti":          {"sig": "padre, hombre que protege", "cat": "sust", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono L-ITTI 'su padre' (Perea Alonso 1942 p. 123) y Goeje 1939 p. 42 «père A iti». `baba` se leía ba-ba (el -ba de lo que vendrá) y era caribe de hombres. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `baba` ('padre, hombre que protege'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},

    # ── Cuerpo ──────────────────────────────────────────────────────────
    "isi":          {"sig": "cabeza, mente, lo alto de", "cat": "sust", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. Goeje 1939 p. 32 (visto en imagen): «tête … išik, išöke … A iši»; Adam 1879 p. 289: lokono «Issihi, tête» → mujeres «ichic»; Goeje p. 105 «graine, tête A isi». La raíz de las dos. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `kabo` ('cabeza, mente, lo alto de'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "akusi":        {"sig": "ojo, mirada, visión", "cat": "sust", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. lokono «Aku-ssi, œil» → mujeres kalinago «n-acou» (Adam 1879 p. 288, visto en imagen); Goeje 1939 p. 33: f aku, A ako. La forma lokona. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `nii` ('ojo, mirada, visión'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "bari": {"sig": "vientre, barriga, interior del cuerpo", "cat": "sust", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 4: se queda y se degrada: ninguna hermana le da pareja. Pasa a HIPOTÉTICO (N1 → A: no llega a dos hermanas); el perfil era2 no la muestrea y llega por plantilla. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-reconstruido`)", "fuente": "caquetío-hipotético"},

    # ── Partículas y conectores ────────────────────────────────────────
    # (permiten construir frases más complejas sin recurrir al español)
    "badia":        {"sig": "y, también, además (conector aditivo)", "cat": "part", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono BADDIA 'y, también' (Perea Alonso 1942 p. 538; ocho atestaciones), sin la geminada. La glosa pierde el 'con' que traía `ka`: la fuente no lo da. `ka` era además homógrafa del prefijo atributivo ka-. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `ka` ('y, también, además, con (conector aditivo)'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "ika":          {"sig": "pero, sin embargo, aunque (contraste)", "cat": "part", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono ICCA 'pero' (Perea Alonso 1942 p. 528) y 'entonces' (p. 513). Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `mara` ('pero, sin embargo, aunque (contraste)'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "bena":         {"sig": "cuando, al momento de (temporal)", "cat": "part", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono BENNA 'cuando' (Perea Alonso 1942 p. 504; cinco atestaciones), sin la geminada. La glosa pierde el 'si' condicional que traía `saa`: la fuente da 'cuando'. ⚠️ A una letra de `bana`. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `saa` ('si, cuando, al momento de (condicional/temp.)'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "kia":          {"sig": "después, luego, más tarde (temporal posterior)", "cat": "part", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 3. lokono KIA 'después' (Perea Alonso 1942 p. 508; cuatro atestaciones). La clave lokono pasa a `kia-lokono`. Una hermana (N1 → A): hipotética; el perfil era2 no la muestrea y llega por plantilla. Sustituye a `naka` ('después, luego, más tarde (temporal posterior)'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "puna": {"sig": "antes, ya, primero (temporal anterior)", "cat": "part", "notas": "núcleo fundacional, forma justificada por cognado en lokono · Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 4: se queda y se degrada: ninguna hermana le da pareja. Pasa a HIPOTÉTICO (N1 → A: no llega a dos hermanas); el perfil era2 no la muestrea y llega por plantilla. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-reconstruido`)", "fuente": "caquetío-hipotético"},
    "kibe":         {"sig": "muy, mucho, bastante (intensificador)", "cat": "part", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), núcleo fundacional grupo 2. Goeje 1939 p. 71 (visto en imagen): «beaucoup f k-ibe … très A k-ibi, k-ibe-n» — las mujeres kalinago y el lokono. ⚠️ Queda a una letra de `kiba` 'piedra'. Dos hermanas con la misma forma o con la correspondencia regular declarada (N1 → A): reconstruida. Sustituye a `wara` ('muy, mucho, bastante (intensificador)'), que citaba «cognado» sin obra. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "uli":          {"sig": "negro, oscuro", "cat": "v_estativo", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. kalinago «uli» 'noir', moderno wiri (Goeje 1939 p. 53, sigla 1 am 133: de origen arahuaco), con el lokono «uele-hi» 'être noir' (Goeje p. 86, columna A) detrás por la misma raíz. El kalinago tal cual: `wele` quedaba a una letra de `were` 'dar'. Estativo (d21.4). Menos de dos hermanas con la misma forma (N1 → A, th.2): hipotética; el perfil era2 no la muestrea y la enseña la plantilla breve (línea VOCES). Sustituye a `mütsia` ('negro, oscuro (< mütsiisü Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "halira":       {"sig": "blanco, claro, luminoso", "cat": "v_estativo", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. lokono «arira, hallira» 'blanc' (Goeje 1939 p. 53, columna A) y el kalinago de MUJERES «alu» (misma línea, sigla f 1 am 132 E; los hombres dicen tamone, caribe). La forma lokona en grafía fonémica. Estativo (d21.4). Menos de dos hermanas con la misma forma (N1 → A, th.2): hipotética; el perfil era2 no la muestrea y la enseña la plantilla breve (línea VOCES). Sustituye a `kasuta` ('blanco, claro, luminoso (< kasüttaa Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "kule":         {"sig": "rojo, color de la sangre", "cat": "v_estativo", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. lokono «CULE-RU Bara = el Mar BERMEJO» (Perea Alonso 1942 p. 108) y achagua «Quirrayi, Colorado» (Neira y Ribero 1762, pliego 49 izq.): l ~ r, el par que C8 declara indecidible. La forma lokona. Las mujeres kalinago dicen pona / lokono bonaro (Goeje p. 77), pero pona queda a una letra de puna y de pana. Estativo (d21.4). Menos de dos hermanas con la misma forma (N1 → A, th.2): hipotética; el perfil era2 no la muestrea y la enseña la plantilla breve (línea VOCES). Sustituye a `sünatü` ('rojo, color de la sangre (< ishasü)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "huda":         {"sig": "morir, muerte", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. lokono «huda» MORIR (Perea Alonso 1942 p. 390, siete atestaciones) y kalinago «hila» 'muerto', «hilaro» 'mourir' (Goeje 1939 p. 86, sigla fhA: arahuaco, en los dos registros). Lokono d = kalinago l es la correspondencia regular (Adam 1879 pp. 288-289): dos hermanas por N1. Verbo que es también su nombre (dc.3). La clave lokono pasa a `huda-lokono`. Dos hermanas (N1 → A, th.2): reconstruida. Sustituye a `outa` ('muerte, fin de la vida (< outaa Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "kake":         {"sig": "vivir, vida", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. kalinago de MUJERES «kakê» 'vif, vivre' y lokono «kake, koke» (Goeje 1939 p. 79, sigla fA; visto en imagen); Adam 1879 p. 290: lokono «Kaku-n, vivre» → mujeres «kake-keili, il vit encore». Dos hermanas, la MISMA forma. Verbo que es también su nombre (dc.3). Dos hermanas (N1 → A, th.2): reconstruida. Sustituye a `kataa` ('vida, aliento vital (< kataa Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "marisi": {"sig": "maíz en mazorca, grano de cosecha", "cat": "sust", "fuente": "caquetío-reconstruido", "notas": "F8 (2026-09-12): acuñación para la simulación sin cita, etiquetada `lokono/proto-arawakan` · N1 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): la forma tiene dos hermanas — «maïs … f mariši, A mariši» (Goeje 1939 p. 65, visto en imagen), la MISMA en las mujeres kalinago y en el lokono: pasa a RECONSTRUIDO y deja de ser una acuñación sin cita. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml (era `caquetío-hipotético`)"},
    ...
    "halikebe":     {"sig": "alegrarse, alegría, contento", "cat": "v_raiz", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. lokono «halli-kebbe» ALEGRAR, GOZAR, DICHOSO (Perea Alonso 1942 pp. 231, 336, 112; cinco atestaciones), el mismo «ahali-kibi» de Goeje 1939 (p. 86, columna A) y el «hallikebe-de, je me réjouis» de Adam 1879 (p. 298). Una hermana, tres fuentes; la kalinago (a)ulabu no comparte forma. Verbo que es también su nombre (dc.3). Menos de dos hermanas con la misma forma (N1 → A, th.2): hipotética; el perfil era2 no la muestrea y la enseña la plantilla breve (línea VOCES). Sustituye a `talata` ('alegría, contento, gozo (< talataa Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "aiima":        {"sig": "rabia, ira, enojo", "cat": "sust", "fuente": "caquetío-hipotético", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D2 de #234: las voces que seguían saliendo del wayuu. el lokono TAL CUAL: «k-aiima» 'fâché' sin el atributivo (Goeje 1939 p. 116, columna A), la misma raíz que Perea Alonso 1942 segmenta en A-IMA-TTU-NNUA IRA (p. 56) y que el kalinago de mujeres dice iam. Dos hermanas en la raíz, la forma de una. Con el atributivo, «ka-aiima» 'tiene enojo' es el lokono k-aiima. `ima` y `aima` no se pueden usar: son sufijos del motor (REGLAS_ZAVALA). Menos de dos hermanas con la misma forma (N1 → A, th.2): hipotética; el perfil era2 no la muestrea y la enseña la plantilla breve (línea VOCES). Sustituye a `jashichi` ('rabia, ira, enojo (< jashichi Wayunaiki)'). Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    "aburi":        {"sig": "tener vergüenza; vergüenza, pudor", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "notas": "Tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»), D3 de #234. Goeje 1939 p. 74 (visto en imagen): «avoir honte f aburi … A (h)aburi» — las mujeres kalinago y el lokono, la misma forma: reconstruida (N1 → A). Verbo que es también su nombre (dc.3). Sustituye a `japü` ('vergüenza, pudor, sonrojo'). ⚠️ HOMÓGRAFO DECLARADO: `aburi` es también una grafía del glosario de Zavala como TOPÓNIMO («Para designar las aguas de un río lleno de arena», lexicon_zavala.TOPONIMOS_ZAVALA), fuera del habla y fuera de 2-lengua/toponimos.yaml: no choca en VOCABULARIO_BASE. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml; decisión: 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml"},
    ...
    "kiba": {"sig": "piedra", "cat": "sust", "fuente": "caquetío-atestiguado", "forma_fuente": "quiva", "notas": "Decisión colisiones D5 (2026-08-31) — HOMÓNIMOS DECLARADOS bajo el mismo lema: kiba-1 piedra (Zavala Reyes 2015 #218 «Quiva» (E); y #92 «Cuiva. Kiba» (PMA) piedra — Arcaya registró la grafía k: el lema fonémico está impreso en la fuente) y kiba-2 ayuda (Zavala #203 «Quiba» (AM), cat v_raiz). La grafía b~v es betacismo colonial: mismo lema fonémico. El sentido piedra lleva el sig activo por la capa toponímica (van Buurt §8 siba/quiba piedra-roca; quibacoa, Todariquiba). Ambas salen del generado: FUSIONADAS_EN_LITERAL del miner · T4 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): pareja en la ESFERA — taíno «siba, Ciba» 'piedra' (Pané; Las Casas vía Coll y Toste 1897 p. 209), de cronista del XVI. Es una de las tres parejas k ~ s (kiba/siba, kabana/sabana, kiwa/sigua) que T11 dejó en «indecidible» con dos apoyos: se vuelve a correr después de la corrida base (T5). Comparanda de la esfera, no evidencia de préstamo ni de parentesco por sí sola; no cambia glosa ni capa. Propuesta: 6-fusion/taino_tradicion_viva_2026-09-24.yaml §el_cruce"},
    ...
    "sawaka":     {"sig": "inframundo, reino de los muertos", "cat": "sust", "fuente": "caquetío-atestiguado", "notas": "van Buurt 2014 §6 «words likely to be of Caquetío origin» (el autor advierte que la lista «has a subjective element»), isla C: «the underworld, the realm of the dead, the beyond»; en papiamento antiguo la expresión baha na sawaka = descender al inframundo, morir; paralelos en taíno y lokono · cc.10 (e) B, cosmovisión marina (2026-09-24, «Ok a todo»): SÓLO SE ANOTA. Es una voz de papiamento que van Buurt atribuye al caquetío «with a subjective element»; por la regla 2 sería `reconstruido`, pero es la raíz del nombre del boratio mayor del elenco (Sawaka) y degradarla mueve el elenco: queda escrito, sin decidir en esta tanda. Y es la tierra de los muertos ABAJO, no mar adentro (6-fusion/cosmovision_marina_2026-09-24.yaml)"},
    ...
    "kumarawa": {"sig": "caracol de las costas de Paraguaná", "cat": "sust", "fuente": "caquetío-atestiguado", "glosa_fuente": "Especie de caracol de las costas de Paraguaná [Alvarado 1921 p.102 s.v. CUMARAGUA]", "notas": "D10 (2026-08-03), grupo 2 — GLOSA CORREGIDA, la palabra sigue siendo caquetía. Alvarado 1921 p.102 es la localización más precisa de todo su glosario para una voz de esta lista: costas de Paraguaná. PREMISA CORREGIDA: la glosa anterior NO estaba sin fuente. La lectura que se descarta —Zavala Reyes 2015 glosario #93 (HB+E) «Ciruela, espuma rosada», fuerza F, y Arcaya 1920 sobre la Relación de Barquisimeto 1579 («mene y cumaragua nombre de la ciruela»)— queda registrada aquí íntegra por D7. Las dos son fuentes históricas y ninguna es moderna: el conflicto es real y D10 adjudica por localización · dp.1.05 y dp.1.08 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), la reapertura de D10, CERRADA SIN MOVER LA GLOSA (minar-fuente §8: si una fuente contradice, se anota; no se reescribe). CITA CORREGIDA: Arcaya 1920 p. 75 (vista en imagen, #216 M6) escribe «viruela», no «ciruela» —la «ciruela» de Zavala #93 es una mala lectura de ella—, y la Tabla A-9 de Oliver (#25, vista en imagen, #213 M1) también da 'viruela / smallpox'. Ojo, regla 3: la viruela llegó con los españoles (1518 en La Española), así que esa acepción es COLONIAL por construcción y la voz pudo nombrar otra cosa antes. Cuarta lectura: Esteves 1989 p. 33 s.v. CUMARAGUAS, «un pequeño cangrejo de caparazón rosada», 'espuma rosada' (#208 FA3) — la costa de Paraguaná otra vez, pero es la etimología de un topónimo, que cc.4 deja hipotética. Se queda la de Alvarado (costas de Paraguaná), la única atestación localizada de la voz.", "forma_fuente": "cumaragua"},
    "busera":     {"sig": "jagua, tinte negro-azulado con que se pinta el cuerpo (Genipa americana)", "cat": "sust", "fuente": "caquetío-atestiguado", "forma_fuente": "busera", "notas": "dp.1.08 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción A: la Tabla A-9 de Oliver verificada (#213 M1). Oviedo t. II p. 300 (oviedo-y-valdes-1852-1855, visto en imagen): «embixados y pintados de bixa y de xagua, que allí llaman busera» — la XAGUA (negro), con la bixa (rojo) al lado como otra cosa. UNA sola atestación: la A-9 de Oliver (n.º 12, buxera, «pintura negra-Genipa») bebe del mismo Oviedo (tesis pp. 283-284). La glosa 'rojo, bija' de la n. 213 del cap. 3 de Oliver es la del EDITOR de Oviedo (t. IV p. 595): la trampa de datihao. Polity: Oliver ASUME que el funeral del díao que describe Oviedo es el de la costa (tesis p. 283: «it is reasonable to assume…»); suposición declarada, no dato. Época: contacto temprano (regla 3). Antes no estaba en el lexicón bajo ninguna grafía (busera, buxera, bujera: 0, medido por M1)"},
    ...
    "jagey": {"sig": "estancar, represar, crear charco artificial", "cat": "v_raiz", "fuente": "caquetío-atestiguado", "notas": "Zavala Reyes 2015, glosario #174 (AM): «Establecer, estancar» [atribución débil: jagüey es panamericano, de origen taíno] · T4 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): pareja en la ESFERA — taíno «jagüey» 'depósito de agua dulce' (Coll y Toste 1897 p. 232) y T xagueye 'citerne naturelle' (Goeje 1939 p. 57), de la tradición viva de Puerto Rico. Comparanda de la esfera, no evidencia de préstamo ni de parentesco por sí sola; no cambia glosa ni capa. Propuesta: 6-fusion/taino_tradicion_viva_2026-09-24.yaml §el_cruce", "forma_fuente": "jaguey"},
    ...
    "kama":       {"sig": "tapir, danta (Tapirus terrestris)", "cat": "sust", "fuente": "caquetío-hipotético", "notas": "Decisión F1 (tanda 2026-08-30) — DEGRADADA: SIN_RASTRO real en las cuatro minerías (auditar_82); el lokono usa otra raíz (firobero). Candidata a reconstruido vía cognado proto-arahuaco *kema. RUTA CORREGIDA 2026-08-31: el Swadesh-100 de Oliver NO trae tapir (medido al minar la serie A-1..A-7); candidatos reales: el vocabulario paraujano completo de Wilbert (Tabla A-1), las voces de fauna de van Buurt, o la comparativa externa (Payne). Ver 6-fusion/tabla_a1_a7_swadesh.yaml · dp.1.08 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)): NO es «SIN_RASTRO». La fila existe en la Tabla A-9 de Oliver (#15, «cama | K-ama | danta | tapir», vista en imagen, #213 M1) y el cap. 2 p. 142 da la fuente: la Relación de BARQUISIMETO de 1579 (Arellano Moreno 1964 p. 183), «cama» o «çama». Regla 4: es de Barquisimeto, no de la polity costera; hay huesos de tapir en los fogones de Túcua (cap. 4 p. 439). NO SUBE de capa hasta tener la Relación (dp.3.01): Oliver la cita sin transcribir la página"},
    ...
    "warawara": {"sig": "caracara, ave rapaz carroñera (Caracara cheriway)", "cat": "sust", "fuente": "caquetío-atestiguado", "notas": "van Buurt 2014 §6 «words likely to be of Caquetío origin» (el autor advierte que la lista «has a subjective element»), islas A/B/C: «the crested Caracara (Caracara cheriway syn. Polyborus plancus)»; respaldo toponímico §7 (Sero Warawara, Warawao, Aruba). La glosa anterior del lexicón («buitre, zamuro (Cathartes curasoica)») era, palabra por palabra, la identificación de Gatschet 1885 (material Pinart 1882), heredada sin saber de dónde venía: Cathartes es Cathartidae (zamuros) y Caracara es Falconidae. D7 registra las dos lecturas: la de 1885 queda aquí, la moderna pasa a la glosa activa · T4 de la tanda de las hermanas (2026-09-24, «Acepto todo lo recomendado»): pareja en la ESFERA — taíno «guaraguao» 'ave de rapiña' (Coll y Toste 1897 pp. 223, 156), de la tradición viva: rapaz, y voz imitativa. Comparanda de la esfera, no evidencia de préstamo ni de parentesco por sí sola; no cambia glosa ni capa. Propuesta: 6-fusion/taino_tradicion_viva_2026-09-24.yaml §el_cruce"},
    ...
    "piragua":    {"es": "piragua, canoa grande de un palo", "fuente": "kalinago", "notas": "Tno. piragua → español piragua · dp.1.11 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), T10 A con M5 C: pasa de `taíno` a `kalinago`. Goeje 1939 la pone en el caribe insular (y sospecha del español «vela»); Bachiller p. 389 la lista como de Borinquen («Bote, Piraguas, B.»), no eyeri: DOS apoyos, no tres (M5 C corrige a T10). Y Goeje avisa de que piragua, kanoa y hamaka viajaron a otras lenguas indígenas por vía del español. Ninguna fuente la llama taína (#198 T10, #219 M5). Sigue en la esfera de contacto: el cambio es de etiqueta", "categoria": "navegacion"},
    ...
    "guabina":    {"es": "guabina, pez de agua dulce (Hoplias malabaricus)", "fuente": "taíno", "notas": "Tno. guabina; pez de ríos y lagunas costeras · dp.1.11 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), T10 A: sube de «sin apoyo» a «atestiguada, libro por localizar» — Bachiller 1883 p. 267 la da dentro de la enumeración de peces de nombre indio de Las Casas (#198 T10); la página de Las Casas no se ha visto", "categoria": "fauna"},
    "bagua":      {"es": "mar", "fuente": "taíno", "categoria": "geografia", "notas": "Entra el 2026-09-24 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), cerrando lo que tf.6 dejó abierto («manatí, tabaco y bagua según su cronista»). Oviedo, Historia general t. I, lib. XIII, cap. IX, p. 436 (oviedo-y-valdes-1851, verificado en imagen, 6-fusion/fuentes_poporo_coro_zayas_2026-09-23.yaml §bagua): «Llaman los indios de aquesta Isla Española á la mar bagua (no digo baygua, porque baygua es aquel barbasco […] sino bagua es el nombre de la mar en esta isla)». El glosario del editor (t. IV p. 594, «BAGUA: mar, piélago. (Lengua de Haiti.)») y Zayas la copian de ahí. Variedad: La Española. El caquetío dice `para` (atestiguado): esto es la voz de la esfera, no un sinónimo"},
    "cobo":       {"es": "cobo, caracol marino gigante (Strombus gigas), trompeta ritual", "fuente": "taíno", "notas": "Tno. cobo; objeto de intercambio arahuaco · cc.10 (g) A (2026-09-24): Pané dice «el cobo es el caracol de mar»; no hay trompeta de caracol taína en el repo (los caracoles de Anglería y de Paria son sonajas). La glosa no se toca (D7): se anota (6-fusion/cosmovision_marina_2026-09-24.yaml)", "categoria": "fauna"},
    ...
    "bejique":    {"es": "bejique, chamán taíno, mediador ritual", "fuente": "taíno", "notas": "Tno. bejique; cognado de piache (Lokono piaye) · dp.1.11 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), T10 A: la duda, anotada — Goeje 1939 da `behiko` como caribe insular usado en Haití y `buhuitihu` como el taíno; Bachiller la da de Cuba y las Lucayas con cita de Las Casas, Apologética p. 436. Los dos intermediarios discrepan (#198 T10)", "categoria": "cosmos"},
    "huracan":    {"es": "huracán, espíritu del viento destructor, ciclón", "fuente": "taíno", "notas": "Tno. hurakán → español huracán; ser sobrenatural arahuaco · cc.10 (g) A (2026-09-24): sin procedencia. Oviedo, Las Casas y Anglería glosan huracán como TORMENTA; la tormenta de Pané es el cemí Guabancex. Lo de «espíritu del viento destructor» no lo dice ninguna primaria del repo. La glosa no se toca (D7): se anota (6-fusion/cosmovision_marina_2026-09-24.yaml)", "categoria": "cosmos"},
    "maboya":     {"es": "maboya, espíritu maligno nocturno", "fuente": "kalinago", "notas": "Tno. maboya; equivalente al buio en cosmología caquetía · dp.1.11 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), T10 A con M5 C: pasa de `taíno` a `kalinago`. Goeje 1939 la da como caribe de Honduras (ma-poya) y Bachiller la publica en la lista eyeri; su único apoyo taíno era Rafinesque. La voz taína para el espíritu del muerto es `hupia` (#198 T10, #219 M5). Sigue en la esfera de contacto: el cambio es de etiqueta", "categoria": "cosmos"},
    ...
    "ma": {
        ...
        "notas": "núcleo fundacional, forma justificada por cognado en lokono · Lok. ma-; prefijo privativo 'sin/carente'; ma-bolheidi-n 'no tirar'; antónimo de ka-; Pet 1987", "fuente": "caquetío-reconstruido",
        "categoria": "gramatica"
    },
    "myn": {
    ...
    "taita": {
        "es": "padre",
        "fuente": "taíno",
        "notas": "Taíno atestiguado: taita; cognado Lok. itti; Brinton 1871 · F8 (2026-09-12): grafía de `fuente` unificada, `taino` → `taíno` · dp.1.11 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), T2.3 (a) — deuda: sin-procedencia-colonial. Su único apoyo es Brinton 1871 p. 13, que cita a Pichardo (diccionario provincial cubano del s. XIX); Bachiller 1883 la pone en su apéndice (C) de voces que pasan por indígenas y vienen de otra parte (propone el vascuence aita); Goeje 1939: 0 (#198 T10). Se deja, anotada",
        "categoria": "parentesco"
    },
    "tuna": {
        "es": "agua, río",
    ...
    "yamosa": {
        "es": "dos",
        "fuente": "taíno",
        "notas": "Taíno atestiguado: yamosa; cognado Lok. biama; Brinton 1871 · F8 (2026-09-12): grafía de `fuente` unificada, `taino` → `taíno` · dp.1.11 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), T9 D2 (B): se deja y se anota — Brinton lee `yamosa`, la Apologética de Las Casas en NBAE 13 p. 538 (verificada en imagen, #194 T9) lee «yamocá»",
        "categoria": "gramatica"
    },
    ...
    "barana":   {"es": "mar, agua extensa", "fuente": "kalinago", "notas": "Garifuna barana = gran cuerpo de agua · CORREGIDO el 2026-09-24 (cc.10 (e) A, cosmovisión marina, «Ok a todo»): la nota decía «cognado de CQ para, LK bara, PA *para», y Goeje 1939 p. 55 da `barana` como la forma de HOMBRES, de origen caribe/tupí (parana); la del lado arahuaco, la que va con el caquetío `para` y el lokono `bara`, es la de MUJERES: balaua (Goeje p. 55), «la mer, balànna, f bálaoüa» (Breton 1666 p. 242, en imagen). 6-fusion/cosmovision_marina_2026-09-24.yaml", "categoria": "geografia"},
    "kasabi-kalinago":   {"es": "casabe, pan de yuca", "fuente": "kalinago", "notas": "Kalinago kasabi; idéntico a LK kasabi, CQ casabe; término central de la identidad cultural. Sufijo -kalinago: colisión de clave con entrada lokono 'kasabi' preexistente · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: el casabe es aleiba / ereba (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "alimentos"},
    ...
    "yuka-kalinago":     {"es": "yuca, mandioca (Manihot esculenta)", "fuente": "kalinago", "notas": "Garifuna yuka; idéntico a LK yuka, CQ yuca, PA *yuka; base alimentaria de la cultura Kalinago. Sufijo -kalinago: colisión de clave con entrada wayunaiki 'yuka' preexistente · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: la yuca es kiere / key (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "flora"},
    ...
    "ikoa":     {"es": "casa, vivienda", "fuente": "kalinago", "notas": "Garifuna ikoa; LK sikoa → ikoa (pérdida s- inicial); PA *isikoa · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: la casa es mana, ubana, obogne… (~ lokono sikoa) (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "arquitectura"},
    ...
    "hamaka-kalinago":   {"es": "hamaca, cama colgante", "fuente": "kalinago", "notas": "Garifuna hamaka; conservado igual que PA *hamaka, CQ hamaca, LK hamaha; préstamo pan-arahuaco al español. Sufijo -kalinago: colisión de clave con entrada wayunaiki 'hamaka' preexistente · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: la hamaca es akat / ekera (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "utiles"},
    ...
    "kasaku":   {"es": "firmamento, bóveda celeste", "fuente": "kalinago", "notas": "Garifuna kasaku; LK kassaku → kasaku (ss→s); sustrato arahuaco en cosmología Kalinago · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: (~ lokono kassaku) (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "cosmos"},
    "hiñaru":   {"es": "mujer (registro femenino kalinago)", "fuente": "kalinago", "notas": "Breton (1665) registro femenino/neutro; LK hianaro; refleja la gramática arahuaca del Kalinago; el 'registro de las mujeres' documentado por los misioneros · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)): la glosa era 'persona, ser humano'; Goeje 1939 la da como 'mujer' en el habla de mujeres (uele / inharu), #219 M5. Es la inharu de Adam p. 289 que la tanda de las hermanas cita junto a iero", "categoria": "parentesco"},
    ...
    "baruwa":   {"es": "hombre (registro masculino Kalinago)", "fuente": "kalinago", "notas": "F8 (2026-09-12): etiqueta vieja `kalinago-caribe-overlay`: forma del registro caribe (overlay) del kalinago · Breton (1665) registro masculino; origen caribe; contraparte de hiñaru (arahuaco); la dualidad baruwa/hiñaru es evidencia del proceso de contacto que generó el Kalinago · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: el par real es uekeli / eyeri 'hombre' y uele / inharu 'mujer'; baruwa no aparece (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "parentesco"},
    ...
    "pira":     {"es": "pez, pescado (forma caribe del Kalinago)", "fuente": "kalinago", "notas": "F8 (2026-09-12): etiqueta vieja `kalinago-caribe-overlay`: forma del registro caribe (overlay) del kalinago · Origen caribe; cf. piraña = pira + aña (diente en Tupí); el overlay caribe dominó el vocabulario de pesca en el registro masculino Kalinago; contrasta con el arahuaco LK itime · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: el pez es aoto (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "fauna"},
    "amourou":  {"es": "guerra, combate", "fuente": "kalinago", "notas": "F8 (2026-09-12): etiqueta vieja `kalinago-caribe-overlay`: forma del registro caribe (overlay) del kalinago · Breton (1665); vocabulario bélico casi exclusivamente caribe en Kalinago; ausencia del término arahuaco equivalente en registro masculino · dp.1.10 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción B — deuda: sin-procedencia. Goeje 1939 no la da: la guerra es ualime (#219 M5; 6-fusion/kalinago_goeje_1939.yaml §cruce_con_el_lexicon). Es el patrón de las nueve «taíno-reconstruido»: una voz vecina con la etiqueta cambiada", "categoria": "guerra"},
...
REGLAS_LOCATIVAS: dict[str, dict] = {
    ...
    "-bana": {
        ...
        "evidencia": "D9 (#38), aprobada 2026-08-30: Zavala Reyes 2015 #26 «Bana (E): "
                     ...
                     "enseñando 'orilla de'",
        "instruccion_agente": (
            "Para un cerro o un sitio alto: 'kidi-bana' = la cumbre del cerro; "
            "'ada-bana' = el cerro arbolado. Para la orilla del mar usa kari."
        ),
    },
}
...
REGLAS_POSESIVAS: dict[str, dict] = {
    ...
    "da-": {
        ...
        "ejemplos": [
            ...
            "da + hamaka = da-hamaka (mi hamaca)",   # era `anüiki`, archivada en tf.5 (2026-09-23)
        ],
    ...
    "wa-": {
        ...
        "ejemplos": [
            ...
            "wa + kanoa = wa-kanoa (nuestra canoa)",   # era `anüiki`, archivada en tf.5
        ],
...
REGLAS_ATRIBUTIVAS: dict[str, dict] = {
    ...
    "ma-": {
        ...
        "ejemplos": [
            ...
            "ma + uni = ma-uni (sin agua, la tierra seca)",   # era `anüiki`, archivada en tf.5
        ],
...
REGLAS_ZAVALA: dict[str, dict] = {
    "-aima": {
        ...
        "instruccion_agente": (
            "Para decir que algo abunda en un sitio, añade -aima: 'hime-aima' "
...
# que ya circulaba.
#
# TANDA DE LAS HERMANAS (2026-09-24): `kuru` se archiva y `ada` 'árbol' pasa a
# ser la voz caquetía (lokono ada = kalinago ara-, Adam 1879 p. 288), así que
# el ejemplo vuelve a la raíz del topónimo que la morfema-001 cita —Adabacoa
# 'todo arboleda'—, con su lema fonémico. ⚠️ La condición de d21.14 («no se ha
# dicho nunca en la base») queda POR MEDIR: la base está en Docker y no se
# pudo consultar al aplicar. `ada` no era voz caquetía y es improbable que
# circulara, pero no está medido.
REGLAS_TOPONIMICAS: dict[str, dict] = {
    "-bakoa": {
        ...
        "ejemplos": ["ada + -bakoa = ada-bakoa (la arboleda, el paraje de árboles)",
        ...
        "instruccion_agente": (
            ...
            "'ada-bakoa' es la arboleda, el paraje de árboles."
        ),
    },
}
...
# dp.1.12 de #222 («Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)), opción J: los formantes de topónimo que Oliver
...
TODAS_LAS_REGLAS = {
    ...
    **REGLAS_ESTEVES,
    **REGLAS_TOPONIMICAS,
    **REGLAS_OLIVER,        # dp.1.12: se pelan, no se enseñan
}
...
# ── TANDA DE LAS HERMANAS (2026-09-24): el núcleo desde el lokono y el habla
# de mujeres kalinago («Acepto todo lo recomendado»). `arima` 'pez' → `hime`
# (lokono, una hermana: hipotética, y esta línea es por donde la ven los 63)
# y `wara` 'muy' → `kibe` (mujeres kalinago k-ibe = lokono k-ibi: dos
# hermanas). La frase dice lo mismo. 6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml
...
EJEMPLO: "Dai diki-kuba hime kibe para. Da-barsure kuburuku. [biro-bana: biro+-bana = cerro de la sal]."
...
def prompt_reglas_breve() -> str:
    ...
    """
    # TANDA DE LAS HERMANAS (2026-09-24): los conectores salen del lokono
    # (badia, ika, bena, kia: una hermana, hipotéticos — esta línea es su
    # única puerta a los 63) y `kibe` de las dos; el ejemplo `kuru` → `ada`.
    # VOCES suma las seis hipotéticas de D2 (#234), que ninguna plantilla
    # enseñaba y el perfil era2 no muestrea: sin esta línea no las vería
    # nadie (6-fusion/decisiones_tanda_hermanas_2026-09-24.yaml
    # §lo_que_aparecio_al_aplicar).
...
CONECTORES: badia (y/también) / ika (pero) / bena (cuando) / kia (después) / danu (ahora) / kibe (muy).
VOCES: ruku (adentro) / kasalini (tener frío) / mautia (la mañana temprana) / saika (bueno) / uli (negro) / halira (blanco) / kule (rojo) / halikebe (alegrarse) / aiima (enojo).

VACÍO LÉXICO → CREA con morfemas entre corchetes:
[forma: raíz+sufijo = significado]
Ejemplo: [ada-bana: ada+-bana = cerro arbolado, loma con árboles]
...
def prompt_reglas_completo() -> str:
    ...
    # TANDA DE LAS HERMANAS (2026-09-24, «Acepto todo lo recomendado»): el
    # núcleo fundacional rehecho desde el lokono y el habla de mujeres
    # kalinago. Donde el caquetío atestiguado ya lo decía, manda él: `kono` →
    # `jusual`, `sima` → `kidi`, `nomi` → `ateri`, `wari` → `iero`, `arua` →
    # `ako`, `buri` → `dare`. Dos hermanas (reconstruidas): ani, aeke, ati,
    # dunku, aita, isi, akusi, kibe, ada, uni, unia, butu. Una hermana
    # (hipotéticas, y ESTA plantilla es su puerta al tier 1): kunu, sile,
    # hiti, kudu, hikihi, hime, wunabu, iti, badia, ika, bena, kia. `kaa`,
    # `maa`, `suka`, `ama`, `bari` y `puna` se quedan, hipotéticas.
    # 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml
    v_raiz = ("kunu (ir) · sile (venir) · kaa (ser/estar) · were (dar, entregar) · maa (hablar) · butu (tomar) · "
              "ani (hacer/construir) · diki (ver) · dunku (dormir) · aeke (comer) · ati (beber) · "
              "jai (oír, escuchar) · aita (saber) · kuburuku (pensar) · jusual (sembrar) · hiti (querer) · kudu (cargar)")
    naturaleza = ("uni (agua) · hikihi (fuego) · kasi (sol) · kati (luna) · unia (lluvia) · "
                  "ada (árbol) · hime (pez) · para (mar) · bara (palo, árbol) · wunabu (tierra) · suka (noche) · "
                  "kidi (cerro) · mautia (la mañana temprana)")
    # tf.5: `wanü` → `wasima` (atestiguada, Zavala #145); `pütchi` archivada sin sustituta.
    personas = "ama (madre) · iti (padre) · dare (hijo/a) · ateri (hombre) · iero (mujer) · wasima (viejo, anciano)"
    # D2 de #234 (tanda de las hermanas): los colores, 'bueno' y el ánimo, que
    # salían del wayuu. `huda` y `kake` son reconstruidas; las otras seis,
    # hipotéticas, y la breve también las enseña.
    animo = ("saika (bueno) · uli (negro) · halira (blanco) · kule (rojo) · "
             "huda (morir, muerte) · kake (vivir, vida) · halikebe (alegrarse) · aiima (enojo)")
    ...
    sustantivos = ("barsure (alma) · buko (represa) · biro (sal) · boratio (piache, jefe) · "
                   ...
                   "ako (comida) · maure (algodón) · urari (curare)")
    # tf.5: `kashi` → `danu`, `yama` → `popoi` (atestiguada, Zavala #201 'ahí'), `sulu` → `ruku`.
    conectores = ("badia (y/también) · ika (pero) · bena (cuando) · kia (después) · puna (antes) · "
                  "danu (ahora) · kibe (muy/mucho) · popoi (ahí) · ruku (adentro)")
    cuerpo = "isi (cabeza) · akusi (ojo) · bari (vientre) · kasalini (frío)"
...
EJEMPLO DE RESPUESTA IDEAL (Tier I):
  "Dai diki-kuba hime kibe para. Suka kaa badia kasi kunu-ba mautia.
   Da-barsure maa: Manaure kunu-ba danu — da-akusi diki-kuba [kidi-bana: kidi+-bana = la cumbre del cerro].
   Bena bui kunu-ba buko, kia dai badia kunu-ba."
  (Vi muchos peces en el mar. La noche está, y el sol vendrá en la mañana.
   ...
   Cuando vayas a la represa, después yo también voy.)
...
MORFOLOGÍA:
  ASPECTO (al final del verbo):
    el verbo solo = lo que pasa ahora: kunu (voy) · dunku (duermo) · kuburuku (pienso)
    -kuba = ya pasó: kunu-kuba (ya fui) · diki-kuba (ya vi) · aeke-kuba (ya comí)
    -ba = vendrá:    kunu-ba (iré) · maa-ba (hablaré) · hiti-ba (querré)
  UN ESTADO ES UN VERBO: lo que el español dice con adjetivo, tu lengua lo PREDICA.
    ...
    se dicen solos en presente y llevan -kuba / -ba igual que kunu o aeke. No son adjetivos: se conjugan.
  UN VERBO ES TAMBIÉN SU NOMBRE, sin marca: jusual es sembrar, la siembra y el sembradío.
  POSESIVOS (prefijos):
    da- = mi:      da-barsure (mi alma) · da-akusi (mi ojo) · da-hamaka (mi hamaca)
  ...
  LOCATIVOS (crear topónimos):
    -bana = cerro, sitio alto de X: kidi+bana = la cumbre · kapu+bana = kapubana, el duende del cerro
...
FUERA_DEL_HABLA: dict[str, dict] = {
    ...
    # ── Tanda de las hermanas (2026-09-24): «Acepto todo lo recomendado» ──────
    ...
    "kono":         {"sig": "sembrar, plantar, cultivar", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 1) · manda `jusual`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/taíno · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la atestiguada ya lo dice: `jusual` (Zavala #180 (AM), «sembrar, siembra, sembradío. Conuco»), caquetío atestiguado, cubre el concepto (d19.c). LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "sima":         {"sig": "cerro, elevación del terreno", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 1) · manda `kidi`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/topónimo (Barquisimeto) · d21.16 (2026-09-21), punto 5 — CONVIVENCIA DECLARADA con `turumako` 'cerro de cima plana, meseta' (atestiguada, Zavala #262 AM). Las dos se quedan y las dos se dicen: por el criterio de la política d19.b NO son par —«cerro, montaña, elevación» contra «cerro, meseta» es solapamiento parcial, no glosa idéntica— y Miguel: «me parece bien que convivan sima y turumako […] al final no pasa nada si hay símiles, en todo lenguaje hay símiles». Se afinan las dos glosas para que dejen de emparejar: aquí la elevación a secas, allí la cima plana. Y el argumento morfológico que cierra la tercera banda de aquel issue: `-bana` es un SUFIJO locativo, no un sustantivo; compone (`kali-bana`, `biro-bana`) pero no nombra, así que «cerro ya se dice con -bana» no se sostiene. El par 6 («sima») deja de estar abierto por esta vía: no se archiva ninguna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la atestiguada ya lo dice: `kidi` (Zavala #212 (AM), «Cerro, altura»), caquetío atestiguado, cubre el concepto (d19.c); estaba abierta en 6-fusion/curacion_glosas_pares_2026-09-19.yaml. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "nomi":         {"sig": "hombre adulto (no título)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 1) · manda `ateri`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la atestiguada ya lo dice: `ateri` (Zavala #17 (GC), «Hombre»), caquetío atestiguado; y ateri va con el eyeri de las mujeres kalinago. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "wari":         {"sig": "mujer adulta (no título)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 1) · manda `iero`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la atestiguada ya lo dice: `iero` (Zavala #163 (GC), «Mujer»), caquetío atestiguado; iero va con lokono hiaeru / mujeres inharu (Adam 1879 p. 289). LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "arua":         {"sig": "alimento, comida, sustento (raíz de 'arawak')", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 1) · manda `ako`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la atestiguada ya lo dice: `ako` (Zavala #4 (E+AM), «comida»), caquetío atestiguado. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "buri":         {"sig": "hijo, hija, criatura, descendiente", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 1) · manda `dare`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la atestiguada ya lo dice: `dare` (Zavala #103, «diente; hijo (extensión metafórica)»), caquetío atestiguado, que ya lleva 'hijo'. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "chaa":         {"sig": "hacer, construir", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `ani`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arahuaco · d21.16 (2026-09-21): la glosa decía «hacer, construir, crear» y ese «crear» la emparejaba con `eroa` «empezar, crear, originar», que es ATESTIGUADA — y no son lo mismo: uno construye, el otro origina. Miguel: «efectivamente pasa lo mismo, son símiles pero que tienen significados distintos también». Se quita «crear»; la entrada no se archiva, porque sin el solapamiento no hay par · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `ani`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "masa":         {"sig": "comer, alimentarse", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `aeke`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `aeke`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "awa":          {"sig": "beber, tomar líquido", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `ati`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `ati`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "suna":         {"sig": "dormir, reposar, descansar", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `dunku`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `dunku`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "panaa":        {"sig": "saber, conocer, entender", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `aita`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `aita`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "kabo":         {"sig": "cabeza, mente, lo alto de", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `isi`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `isi`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "nii":          {"sig": "ojo, mirada, visión", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `akusi`", "notas": "núcleo fundacional, forma justificada por cognado en lokono-cogn · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `akusi`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "wara":         {"sig": "muy, mucho, bastante (intensificador)", "cat": "part", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `kibe`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `kibe`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "kuru":         {"sig": "árbol, madera, tronco", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `ada`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/proto-arawakan · d21.16 (2026-09-21), punto 4 — CONVIVENCIA DECLARADA con `bara` 'palo, árbol' (atestiguada, Zavala #29 E). No es un bug de curación ni un par de la política «manda la atestiguada»: es VARIACIÓN REAL, y las dos se quedan. La distinción que las sostiene es la que la propia pareja de glosas enseña — `bara` es el PALO CORTADO (y su reduplicada atestiguada `barabara` es la madera dura, Zavala #30) y `kuru` el ÁRBOL VIVO con su madera y su tronco. Miguel: «al final no pasa nada si hay símiles, en todo lenguaje hay símiles» · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `ada`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "duna":         {"sig": "agua (corriente, bebible)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `uni`", "notas": "núcleo fundacional, forma justificada por cognado en garifuna/lokono · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `uni`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "kaya":         {"sig": "lluvia, agua del cielo", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `unia`", "notas": "núcleo fundacional, forma justificada por cognado en lokono (juya-cogn) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `unia`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "taa":          {"sig": "tomar, coger, recibir", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 2) · la sustituye `butu`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arahuaco · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `butu`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "naa":          {"sig": "ir, moverse hacia", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `kunu`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arahuaco · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `kunu`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "waa":          {"sig": "venir, aproximarse", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `sile`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arahuaco · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `sile`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "raka":         {"sig": "querer, desear, necesitar", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `hiti`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `hiti`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "rua":          {"sig": "cargar, transportar, llevar", "cat": "v_raiz", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `kudu`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `kudu`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "amana":        {"sig": "fuego, lumbre, brasa", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `hikihi`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `hikihi`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "arima":        {"sig": "pez, pescado", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `hime`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `hime`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "dali":         {"sig": "tierra, suelo, polvo", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `wunabu`", "notas": "núcleo fundacional, forma justificada por cognado en garifuna/proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `wunabu`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "baba":         {"sig": "padre, hombre que protege", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `iti`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `iti`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "ka":           {"sig": "y, también, además, con (conector aditivo)", "cat": "part", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `badia`", "notas": "núcleo fundacional, forma justificada por cognado en proto-arawakan · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `badia`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "mara":         {"sig": "pero, sin embargo, aunque (contraste)", "cat": "part", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `ika`", "notas": "núcleo fundacional, forma justificada por cognado en lokono/garifuna · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `ika`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "saa":          {"sig": "si, cuando, al momento de (condicional/temp.)", "cat": "part", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `bena`", "notas": "núcleo fundacional, forma justificada por cognado en lokono · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `bena`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "naka":         {"sig": "después, luego, más tarde (temporal posterior)", "cat": "part", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · núcleo fundacional (th.7, grupo 3) · la sustituye `kia`", "notas": "núcleo fundacional, forma justificada por cognado en lokono · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Acepto todo lo recomendado»): citaba un cognado sin obra ni página y casi ninguna hermana le daba pareja (6-fusion/inventario_nucleo_fundacional_2026-09-23.yaml); la sustituye `kia`, de las hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/nucleo_fundacional_hermanas_2026-09-24.yaml"},
    "anasa":        {"sig": "bueno, bien, bello (< anasü Wayunaiki)", "cat": "adj", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `saika`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki-cogn`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `saika`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "mütsia":       {"sig": "negro, oscuro (< mütsiisü Wayunaiki)", "cat": "adj", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `uli`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki-cogn`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `uli`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "kasuta":       {"sig": "blanco, claro, luminoso (< kasüttaa Wayunaiki)", "cat": "adj", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `halira`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki-cogn`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `halira`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "sünatü":       {"sig": "rojo, color de la sangre (< ishasü)", "cat": "adj", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `kule`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki/lokono`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `kule`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "outa":         {"sig": "muerte, fin de la vida (< outaa Wayunaiki)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `huda`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki-cogn`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `huda`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "kataa":        {"sig": "vida, aliento vital (< kataa Wayunaiki)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `kake`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki/lokono`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `kake`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "talata":       {"sig": "alegría, contento, gozo (< talataa Wayunaiki)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `halikebe`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki-cogn`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `halikebe`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "jashichi":     {"sig": "rabia, ira, enojo (< jashichi Wayunaiki)", "cat": "sust", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · tanda de las hermanas · D2 (th.2): derivada del wayuu · la sustituye `aiima`", "notas": "F8 (2026-09-12): forma derivada de la hermana que cita la glosa, etiquetada `wayunaiki-cogn`; entra en la deuda de D11 fase 3 (núcleo reconstruido desde el wayuu) · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D2, «Acepto todo lo recomendado»): la reconstruyeron desde el wayuu y la campaña de las voces wayuu no la vio (su nota decía «deuda de D11», no «DEUDA D11»); la sustituye `aiima`. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "alaain":       {"sig": "amor, afecto, querer profundo (raíz raka-)", "cat": "sust", "fuente": "caquetío-hipotético", "archivada": "2026-09-24 · tanda de las hermanas · D3 (th.3): etiquetada wayuu, sin cita · sin sustituta", "notas": "F8 (2026-09-12): forma sin cita ni derivación declarada, etiquetada `wayunaiki/lokono` · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D3, «Acepto todo lo recomendado»): sin sustituta: 'amar' es mujeres kalinago inši / lokono ansi (Goeje 1939 p. 95), dos hermanas, pero el grupo ns no pasa la fonotáctica atestiguada. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    "japü":         {"sig": "vergüenza, pudor, sonrojo", "cat": "sust", "fuente": "caquetío-hipotético", "archivada": "2026-09-24 · tanda de las hermanas · D3 (th.3): etiquetada wayuu, sin cita · la sustituye `aburi`", "notas": "F8 (2026-09-12): forma sin cita ni derivación declarada, etiquetada `wayunaiki/lokono` · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, D3, «Acepto todo lo recomendado»): la sustituye `aburi` 'tener vergüenza', de dos hermanas. LA CAPA NO SE TOCA. Propuesta: 6-fusion/kalinago_mujeres_2026-09-23.yaml"},
    # cc.12 aplicado el mismo día: la última derivada del wayuu, sin marca de deuda.
    "ana": {"es": "diseño", "fuente": "caquetío-reconstruido", "archivada": "2026-09-24 · D11 / cc.12 · derivada del wayuu que ninguna campaña había marcado", "notas": "núcleo fundacional, forma justificada por cognado en wayunaiki · Way. ana; *ana (proto-caquetio); Captain & Captain 2005 · ARCHIVADA DEL HABLA 2026-09-24 (tanda de las hermanas, «Ok a todo, que no quede ninguna tarea pendiente»): la reconstruyeron desde el wayuu (Captain & Captain 2005) y su nota no decía «DEUDA D11», así que ni tf.5 ni #234 la vieron; la encontró el barrido de notas duplicadas. cc.12: «no podemos seguir teniendo reconstrucciones desde el Wayu». Sin sustituta: ninguna hermana da 'diseño'. LA CAPA NO SE TOCA. No es el sufijo `-ana`, que sigue en las reglas", "categoria": "comunicacion"},
}
...
# dp.1.04 (#222, «B recortada antes de la base»; «Ok a todo, que no quede ninguna tarea pendiente» (Miguel, 2026-09-24)): las
...
def prompt_refuerzo(score: float, palabras_usadas: list) -> str:
    ...
    # TANDA DE LAS HERMANAS (2026-09-24): las mismas listas con el núcleo
    # rehecho desde las hermanas, voz por voz y en el mismo orden (`suna` →
    # `dunku`, `masa` → `aeke`, `awa` → `ati`, `panaa` → `aita`, `naa` →
    # `kunu`; `ka`/`mara`/`saa`/`naka`/`wara` → `badia`/`ika`/`bena`/`kia`/
    # `kibe`; `duna`/`amana`/`arima`/`kuru` → `uni`/`hikihi`/`hime`/`ada`).
    verbos = [p for p in ["diki","dunku","aeke","ati","jai","aita","kuburuku","kunu","maa","kaa"] if p not in palabras_usadas]
    conect = [p for p in ["badia","ika","bena","kia","danu","kibe","popoi","puna"] if p not in palabras_usadas]
    sust   = [p for p in ["barsure","uni","hikihi","hime","kasi","suka","bara","ada"] if p not in palabras_usadas]
    sug_verbos = ", ".join(verbos[:4]) if verbos else "diki, dunku, aeke, jai"
    sug_conect = ", ".join(conect[:4]) if conect else "badia, ika, danu, kibe"
    sug_sust   = ", ".join(sust[:3]) if sust else "barsure, uni, hime"
    ...
    if score < 2.0:
        ...
        return (
            ...
            f"Prefijos posesivos: da-barsure, wa-uni, ma-ako. "
