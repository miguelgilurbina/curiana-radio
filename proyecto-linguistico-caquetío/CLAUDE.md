...
| **Una acuñación puede ser el ejemplo del prompt** | `kali-bana` es el EJEMPLO literal de `_IDENTIDAD_LINGUISTICA` (`curiana_orchestrator_v2.py:161`) y `warawara` está en `VOCABULARIO_BASE`: las dos se registraron como acuñaciones del día 1 de la serie C y `kali-bana` salió como «adoptada en dos ámbitos» sin haber viajado. El motor ya tiene la lista (`_FORMAS_EXCLUIDAS`, 5.898 formas) pero **la registración no la respeta**, y no se arregló: medido sobre las 72 respuestas del run `b7bc51dc` (el replay reproduce la base 72/72), rechazarlas en `registrar_neologismo` deja `neologisms_proposed` igual pero **mueve `score` en 4** —la forma se oficializa durante el día y una adoptada entra en `palabras_activas`, que es lo que el scorer reconoce—. **El scorer no se toca**: la lista, la medición y las tres opciones quedan en `6-fusion/medicion_formas_de_plantilla_2026-09-18.yaml` y `6-fusion/issues-pendientes/formas-de-plantilla-en-la-registracion-2026-09-18.md`, para un corte de serie |
