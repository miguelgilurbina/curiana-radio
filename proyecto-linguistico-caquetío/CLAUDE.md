...
| `curiana_lexicon` | vocabulario + reglas + prompts + `score_linguistico()`. Las dos vías comunitarias del prompt (V1 propuestas, V2 adoptadas) reciben `ambito` y `LexicoComunitario` guarda dónde se propuso y dónde se adoptó cada forma (`situar()` es quien se lo dice). Vive aquí también la **puerta única** de lo que la plantilla enseña (`IDENTIDAD_LINGUISTICA`, `FORMAS_DE_PLANTILLA`, `es_forma_de_plantilla`), que el orquestador y `analizar_nodos` importan |
