...
| **`lexicon_zavala.py`, `lexicon_a2.py` y `lexicon_achagua.py` son generados Y se importan** | Regenerarlos **cambia `score_linguistico()`** (zavala: habla; a2: columnas de comparación paraujano/lokono, D11; achagua: la comparanda de Neira y Ribero 1762, generada desde el YAML de la transcripción — se corrige el YAML, no el módulo). ⚠️ Los otros `lexicon_*.py` NO los importa el motor, pero **sí el tooling** (`generar_tablero`, `auditar_82`, `migrar_toponimos` — medido 2026-08-15): no se pueden mover de `curiana_sim/` sin romperlo. **Y sus generadores miden contra el lexicón con dos cuidados** (2026-09-24): excluyen lo que el propio módulo fusionó **por identidad de la entrada**, no por clave, y **cuentan lo archivado (`FUERA_DEL_HABLA`) como ocupado**. Sin lo primero el achagua renombraba sus 3.569 claves con `-achagua` y la A-2 reimportaba una clave que otra capa había tomado; sin lo segundo, archivar una voz dejaba que una comparanda la capturara (`pia` paraujano) o destapaba la entrada de Zavala que casaba con ella por un homógrafo falso (`baja` 'caño' tras `baba` 'padre'). Regenerar antes de tocar nada y comprobar que sale igual |
| **El mundo nombra voces del lexicón, y su verificador no es guardián** | `6-fusion/sitios_era2.yaml` y `clima_era2.yaml` dicen la voz caquetía de cada cosa con `voz_caquetia: {clave, capa}`, y `[Tu tierra]` la enseña («Lo decís duna.»). Archivar o re-etiquetar una voz obliga a repuntarla ahí —y en los `palabra_lexicon` del corpus—: la tanda de las hermanas lo encontró por el pre-vuelo (14-20 de 126 prompts), y `6-fusion/scripts/verificar_sitios_era2.py` daba 22 fallos sin que ningún guardián lo viera. Correrlo a mano en cualquier corte que toque el lexicón |
...
| **El préstamo de esfera no es una fuga** | `ESFERA_DE_CONTACTO` (taíno, kalinago, paraujano, caribe-continental, jirajaroide) se mide aparte en `prestamos_de_esfera` y NO penaliza (decisión 2026-09-15). Hablar wayuu o lokono sí penaliza: son el andamio de la reconstrucción, y verlos haría circular la medición. Sólo el tier 1 recibe el bloque `[Voces de fuera]`. Se persiste en `loanword_uses` (con tier y día), NO en `word_uses`, que es sólo caquetío y ningún lector suyo filtra por lengua (run c6837386, 2026-09-16); se lee con `analizar_runs.py --prestamos`. **Y el producto de la esfera ES la esfera** (Miguel, 2026-09-17): casabe, yuca, maíz, batata, papaya son voces taínas del lexicón que el castellano también usa, y por eso un agente propuso sacarlas de `prestamos_de_esfera` como «hispanismos» (#160, mergeado por el escriba leyendo una pregunta como un ok, y revertido la misma tarde). No: son los conceptos y productos que circulaban por la esfera de influencia, el caquetío no tiene voz propia para casabe, y que un agente lo diga es la esfera hablando. Medido en `6-fusion/medicion_hispanismos_loanword_uses_2026-09-17.yaml`: 111 de 161 formas-respuesta con préstamo son cinco de estas voces — así es como se ve la esfera en el habla, no un ruido que filtrar |
| **La voz de la esfera se enseña y se guarda con su forma indígena, no con la castellana** | «La etiqueta manda» (Miguel, 2026-09-18). Que el producto de la esfera SEA la esfera (fila de arriba) no obliga a que circule con grafía castellana. `FORMA_DE_LA_ESFERA` (`curiana_lexicon.py`) mapea la clave castellana a la clave indígena **que ya existe** en el lexicón —casabe→cazabi, maíz→maisi, cacique→cacike, bohío→bohio— y `SIN_FORMA_DE_LA_ESFERA` lista las que no tienen gemela y por tanto **no se enseñan** hasta que Miguel fusione la forma propuesta (atestiguada o retroabstraída a la manera de `matakán`). Medido: 24 de las 133 voces de la esfera llevan marca castellana (4 con gemela, 6 para Miguel, 14 que se quedan porque la grafía ES la transcripción de la fuente — `yuca`, `caiman`, `chighe`); `[Voces de fuera]` pasa de 50 candidatas a 43. ⚠️ **El scorer no se toca**: `score_linguistico()` sigue devolviendo la clave CASTELLANA en `prestamos_de_esfera` —es la que reconoce, y el agente que escribe «casabe» cuenta—; se normaliza al GUARDAR (`save_loanword_uses`: `word` = forma de la esfera, `forma_dicha` = lo que escribió, migración `20260918000000`) y al LEER (`analizar_runs.py --prestamos`). Los runs viejos no se reescriben. Tabla, cita y propuestas en `6-fusion/descastellanizar_esfera_2026-09-18.yaml`; se mide con `6-fusion/scripts/medir_descastellanizar_esfera.py`, que compara contra el módulo de HEAD |
...
| **ESTAR no es VER: son dos puertas** | `ambito_de(agente, state)` dice **dónde está** (el registro vía `situar`, `[Aquí estás]`, `presencias`, el campo) y `ambito_visible_de(agente, state)` dice **qué ve** (V1/V2/V3/V4 y `[Lo que se dijo aquí]`). Coinciden todos los días menos el de **Capubana**, donde la primera devuelve `"Capubana"` —allí está la gente, y así lo ven `adoptados_en_dos_ambitos` y el mapa— y la segunda devuelve `None`, que es lo que las cuatro vías ya entienden como «sin filtro»: la UNIÓN de todos los ámbitos, sin un parámetro nuevo en `LexicoComunitario`, `CampoLexico` ni `CompetenciaLexica`. Antes del 2026-09-18 el cerro **juntaba cuerpos y no ámbitos**: el día 3 era un lugar más, con el léxico de sus dos ocupantes del día 1 (71 formas), V2 vacía y las tres rivales de «las cuentas» a 0 — lo contrario del diseño §4 y la decisión p7. Quien filtre algo por ámbito llama a la segunda; quien registre dónde pasó algo, a la primera |
| **La competencia léxica no sobrevivía la noche** | `CompetenciaLexica` se creaba nueva en cada `auto_mode` y no estaba en `curiana_koine.json`, mientras `referentes_introducidos` (que sí se heredaba) impedía re-presentar el referente: **ninguna disputa podía durar más de un día** y, con un run = un día, por competencia no podía fijarse jamás una entrada de koiné (día 1 de la serie C: 16 variantes de «las cuentas» en 7 ámbitos, líder al 12,2 % contra un umbral del 55 %). Desde el 2026-09-18 `guardar_koine`/`cargar_koine` la persisten entera —referentes abiertos y fijados, variantes con su soporte, el ámbito de cada proponente— y `--continuar` la recupera; sin `--continuar` empieza vacía, y un JSON sin la llave carga con la competencia vacía. `cargar_koine` devuelve **tres** cosas: `(idiolectos, campo, competencia)` |
| **Una acuñación puede ser el ejemplo del prompt** | `naa-ni` es el EJEMPLO literal del bloque de morfología y `warawara` está en `VOCABULARIO_BASE`; el de `IDENTIDAD_LINGUISTICA` era `kali-bana` y hoy es `biro-bana` (fila de abajo). Las tres se registraron como acuñaciones de la serie C, y el día 2 `kali-bana` iba **ganando** «las cuentas» 15,8 contra 3,1. Desde el corte de serie del 2026-09-18 («dale pues con A») **el motor las rechaza**: `curiana_lexicon.FORMAS_DE_PLANTILLA` —vocabulario base + lo que enseñan las cinco plantillas estáticas (identidad, reglas completa y breve, refuerzo, rescate), construida desde ellas y nunca a mano— es **UNA puerta**, la que usan `LexicoComunitario.registrar_neologismo()`, `CompetenciaLexica.proponer()`, `_FORMAS_EXCLUIDAS` y `analizar_nodos`. Lo rechazado no se registra, no compite, no puede adoptarse y no sale en el diccionario de cierre; el rechazo **se cuenta y se dice** al cerrar el run. NO entra en la puerta lo que el prompt le DEVUELVE al agente de la comunidad (V1, V2, V3, `[Lo que se dijo aquí]`, `[Tu manera de hablar]`): eso es el mecanismo que se mide. **El scorer no se tocó**, pero el corte sí mueve `score` (8 de 144 respuestas, |Δ| máx 0,30): una adoptada entra en `palabras_activas`. Medido en `6-fusion/medicion_formas_de_plantilla_corte_2026-09-18.yaml` (control 144/144); la serie C se re-corre desde el día 1 |
| **Cerrarle la puerta a la FORMA no se la cierra al MOLDE** | El corte del 09-18 impidió que `kali-bana` se registrara, pero la plantilla **seguía enseñándola** y los compuestos con la misma raíz nunca entraron en la puerta: en la serie C limpia se dijeron `kali-mara-bana` 12 veces, `kali-ubana-gua` 12, `kali-naa-iro` 10 y `kali-raku-biro` 5, y el molde `kali-…-bana` lleva **233 respuestas** en toda la base sin que ninguna plantilla lo enseñe. Desde el corte del **2026-09-19** («Vale») el ejemplo de `IDENTIDAD_LINGUISTICA` es `[biro-bana: biro+-bana = cerro de la sal]`: `kali` era la forma RECONSTRUIDA del par «sol» y con el muestreador NEUTRO (60 vs. 63 de 1.260) el uso iba 232 a 1 — todo el desequilibrio era el ejemplo. La puerta **se mueve sola** (se construye llamando a las plantillas): sale `kali-bana`, entra `biro-bana`, 5.945 formas antes y después. Dos consecuencias declaradas: `kali-bana` **vuelve a poder acuñarse** (vetarla por haber sido ejemplo es otra decisión, sin tomar — y desde la tanda de la base, 2026-09-23, ya NO: `kali` está archivada y una raíz archivada no avala en la puerta, db.1) y **la era 1 deja de ser byte a byte** — la identidad es UNA constante y los dos mundos la leen. El prompt se mueve +2 caracteres en los 123 (r = −0,48: se mide aunque parezca cosmético). Medido en `6-fusion/medicion_ejemplo_identidad_2026-09-19.yaml`; ojo con «nunca se ha dicho» medido sobre `word_uses`, que sólo tiene lo que el scorer reconoce (`biro-bana`: 0 ahí, 198 respuestas en el texto). Decisión en `6-fusion/decisiones_tanda_2026-09-19.yaml`; la serie C limpia se re-corre otra vez |
| **Donde hay forma atestiguada, manda la atestiguada** | «Sí o sí tenemos que usar los atestiguados por sobre los reconstruidos, por lo menos la parte caquetía» (Miguel, 2026-09-19). Es una POLÍTICA, no una respuesta par a par. **El criterio, que es lo que decide los casos dudosos**: «mismo significado» = **glosa normalizada idéntica** y sólo ésa (NFC + minúsculas, fuera paréntesis/corchetes/diacríticos/artículo inicial; **sin diccionario de sinónimos**); el **solapamiento parcial NO es par**, es una glosa mal afinada y va a curación; «atestiguado» = la etiqueta `caquetío-atestiguado` **más su cita** (regla 8), porque la etiqueta sola no atestigua nada. La derivada **se archiva**: sale de `VOCABULARIO_BASE`, entra en `FUERA_DEL_HABLA` con forma, glosa, `notas` entera y **su capa intacta** — archivar no es borrar y **tampoco es degradar**, la etiqueta dice de dónde viene la palabra y el archivo dice si la comunidad la habla. ⚠️ **Sólo muerde donde EXISTE rival atestiguado**: los aspectos y la mayoría de las reconstruidas no tienen rival y se quedan enteras — esto no es una poda del núcleo (`test_politica_atestiguado_manda.py` (e) lo fija). ⚠️ **Y los pronombres SÍ tienen rival**, corregido el 2026-09-21 (d21.10): `kudanga` y `kuté` son caquetío-atestiguados con cita (Zavala p. 73 vía Arcaya). No los archiva nadie porque **la glosa no es idéntica** —'usted formal' ≠ 'tú'—, así que no es un caso de la política sino el caso barato de al lado: **se reparten registros** y entran los dos al prompt. La frase «los pronombres no tienen rival atestiguado» de `2-lengua/lexicon.md` era inexacta y está corregida. Aplicada a siete pares: `kasi`>`kali`, `were`>`paa`, `jai`>`kira`, `kati`>`kasha`, `para`>`habo`, `juri`>`joutai`, `etamo`>`mülia`. **`FUERA_DEL_HABLA` pasó a ser PUERTA**: `FORMAS_DE_PLANTILLA` lo incluye, porque si no, archivar una voz la sacaba también de la puerta y la dejaba volver al día siguiente como acuñación (`kali`, 2.321 usos, habría vuelto la primera). Dos costes declarados: **`paa` era raíz verbal del núcleo** —56 formas, 1.222 usos, 45 formas y 1.179 usos dejan de contar— pero **no deja hueco funcional**, `were` es `v_raiz` atestiguada y el paradigma se muda de raíz; y **`kasi` 'sol' y `kashi` 'ahora' colisionan** bajo `fonemizar` — el MOTOR no las confunde en ningún sitio (lookup exacto en `_familia_de_token`, token literal en el filtro de nombres, pertenencia a un set en la competencia) pero desde la tanda están las dos en `prompt_reglas_completo`. El scorer NO se tocó y aun así **el score se mueve**: 62 de las 216 de la serie C limpia, todas a la baja, |Δ| máx 0,80 y Δ medio de las que cambian −0,25; el score medio pasa de 7,5977 a 7,5264. Medido en `6-fusion/medicion_politica_atestiguado_manda_2026-09-19.yaml`; decisión en `6-fusion/decisiones_tanda_2026-09-19.yaml` (d19.b); lo que queda abierto —`sima` y once glosas— en `6-fusion/curacion_glosas_pares_2026-09-19.yaml`. **Y el PARA QUÉ de la capa derivada, que es el criterio de los dudosos** (Miguel, 2026-09-20, d19.c): «al final no pasa nada si hay símiles, en todo lenguaje hay símiles, pero lo importante es que usemos sí o sí lo atestiguado y **lo reconstruido lo usemos para nuestros blind spots**». La reconstrucción cubre los huecos sin atestación, no duplica lo documentado: dos voces ATESTIGUADAS para lo mismo son un dato y se quedan las dos; lo que se corrige es la reconstrucción compitiendo con la atestación, que es una sinonimia que inventamos nosotros |
| **Una raíz que no está en el lexicón no es caquetía, por muchos afijos propios que lleve** | `_familia_de_token()` acababa en `return "caquetío"` para todo lo que no encontrara: la razón escrita era buena —«es una acuñación comunitaria, lengua propia»— y vale para `kasi-nii-bana` (raíz atestiguada + dos afijos declarados). No vale para `lumina-bana-iro`, del latín *lumina*: en el brazo de CONTROL de la serie C (2026-09-20) fue la segunda forma más fuerte de la disputa de las cuentas (36,3) y `lumina-bana-uco` **fijó** el cometa en el diccionario koiné. El propio agente lo declaró en su glosa: «lumina: cosa-que-brilla (del español, pero transformada en caquetío)». ⚠️ **No es lo mismo que `neologismo_valido()`**: aquél mira la FORMA (blocklist castellana, marcadores ortográficos, bigramas) y `lumina` pasa sus tres capas; éste mira la PERTENENCIA. **La regla es de RAÍZ**: se quitan de los bordes los afijos que `TODAS_LAS_REGLAS` declara caquetíos y lo que queda es el núcleo (`nucleo_de_token`); si ningún segmento del núcleo es voz del lexicón —`VOCABULARIO_BASE`, `FUERA_DEL_HABLA`, `_RAICES_VERB`, o la clave sin su desambiguador de lengua— la raíz no es de ninguna parte (`es_raiz_de_ninguna_parte`). Se comprueba **antes** que los candidatos, porque el último de ellos es un legado (`tok.split("-",1)[1]`) que con raíz ajena resolvía por el SUFIJO (`pütshi-bana` salía caquetío por `bana` 'hígado'). **Tres puertas y una regla**: no se registra (`registrar_neologismo`, cuenta en `rechazos_de_raiz`, aparte del de plantilla), no compite (`CompetenciaLexica.proponer`) y no cuenta como forma emergente (`PUERTA_DEL_RECUENTO`, que sustituye a `FORMAS_DE_PLANTILLA` en `_FORMAS_EXCLUIDAS` y en `analizar_nodos.formas_excluidas()` — no es un set, es un predicado con `__contains__`). En la base: `source_language` pasa a **`desconocida`** (scorer) o **`acuñada`** (lo que la respuesta declaró, `lengua_de_acunacion`) — la fila se escribe igual, un rechazo callado es un dato perdido. **El scorer NO se tocó y aun así el score se mueve** (corte, punto 11 de la bitácora): 8 de 216 en el brazo con escena y 43 de 216 en el control. El veredicto CONVERGE **se sostiene en los dos**. Tamaño del agujero en toda la base: 156 de 11.390 pares forma-run, 910 de 93.657 usos (0,97 %) — con dos falsos positivos declarados que NO se cuentan: el compuesto que el desafijador viejo no partía (563/4.700) y la voz que ERA canon el día del run y hoy no (553/3.210, casi toda era 1). Medido en `6-fusion/medicion_raices_de_ninguna_parte_2026-09-20.yaml`; se lee por run con `analizar_runs.py --raices` y `analizar_nodos.py`. Abierto: `6-fusion/issues-pendientes/raiz-inventada-puede-un-pueblo-inventar-una-raiz-2026-09-20.md`. **Tres matices desde la tanda de la base** (2026-09-23, db.1 A + C y db.6): la **casi-raíz** de 6 letras o más que difiere en UNA de una raíz caquetía viva se perdona (`pütshi`→`pütchi`; con 4 o 5 se colaban palabras distintas, medido); en la PUERTA —registro, competencia, recuento— una raíz **archivada no avala** (`es_raiz_de_ninguna_parte(forma, archivadas_avalan=False)`: `kira` 'brillo' pasaba por `kira` 'escuchar'), mientras el clasificador sigue leyéndola; y en el turno en que se nombra a un ANIMAL una **raíz onomatopéyica** con forma caquetía entra (`raiz_onomatopeyica_candidata`, `RAICES_ONOMATOPEYICAS`, estado del run que `LexicoComunitario.load()` reconstruye) |
...
python analizar_runs.py --raices  # cuánto de lo guardado como «caquetío» tiene la
                                  # RAÍZ fuera del lexicón, por era, serie y run
                                  # (relectura; la base NO se reescribe)
...
#                           (curiana_*.json en curiana_sim/; la koiné son idiolectos, campo Y la
#                           COMPETENCIA abierta desde el 2026-09-18: una disputa sin resolver sigue
#                           viva al día siguiente y puede llegar al umbral); la config dice
#                           de qué run viene
...
#                           Ese día TODOS VEN TODO: las cuatro vías y [Lo que se dijo aquí] se
#                           sirven con la UNIÓN de los ámbitos (ambito_visible_de → None), mientras
#                           lo que se propone o se adopta se registra en "Capubana". Antes del
#                           2026-09-18 el cerro era un lugar más y no re-exponía nada
...
| `curiana_lexicon` | vocabulario + reglas + prompts + `score_linguistico()`. Las dos vías comunitarias del prompt (V1 propuestas, V2 adoptadas) reciben `ambito` y `LexicoComunitario` guarda dónde se propuso y dónde se adoptó cada forma (`situar()` es quien se lo dice). Vive aquí también la **puerta única** de lo que no puede contar como forma emergente, y son dos reglas: lo que la plantilla enseña (`IDENTIDAD_LINGUISTICA`, `FORMAS_DE_PLANTILLA`, `es_forma_de_plantilla`) y lo que tiene la raíz fuera del lexicón (`nucleo_de_token`, `es_raiz_de_ninguna_parte`). Las dos juntas son `PUERTA_DEL_RECUENTO`, que el orquestador y `analizar_nodos` importan — un predicado con `__contains__`, no un set: las formas de plantilla se enumeran y las raíces posibles no |
| `curiana_agents` | los 60 personajes |
| `curiana_koine` | idiolectos, competencia léxica, métricas de convergencia. `CampoLexico` está partido por ámbito (`{lugar: pesos}`, el global es la suma) y `prompt_competencias(ambito=…)` sólo muestra las formas rivales propuestas allí. `guardar_koine`/`cargar_koine` persisten los tres: idiolectos, campo y **competencia** (`cargar_koine` devuelve una tupla de 3 desde el 2026-09-18) |
...
| `curiana_escena` | **dónde está cada uno** (era 2, `--escena`) y, desde el 2026-09-17, **qué oye allí**. Dos puertas y no una (2026-09-18): `ambito_de(agente, state)` = dónde ESTÁ —una cadena, no un booleano: mañana será adónde se movió por potestad— y `ambito_visible_de(agente, state)` = qué VE, que es la misma salvo el día de Capubana, donde devuelve `None` («sin filtro»). Las dos dan `None` sin escena. Dos bloques: `bloque_aqui_estas` (≤ 200) y `bloque_lo_que_se_dijo_aqui` (≤ 280, el momento anterior). Lee `curiana_escena_era2.py` (GENERADO desde `6-fusion/escena_era2.yaml`, que deriva del elenco). Determinista: no toca el RNG del motor |
...
Pronombres: dai (yo), bui (tú), lihi (él) / tuhu (ella, ello), waya (nosotros),
            ...
            reconstruidos (lokono = habla de mujeres kalinago, desde la
            tanda de las hermanas, 2026-09-24), waya/naya reconstruidos
            (achagua). taya, pia y nüma están archivados.
Núcleo:     las voces básicas salen del lokono y del habla de mujeres
            kalinago desde la tanda de las hermanas (2026-09-24, «Acepto
            todo lo recomendado»): kunu ir, sile venir, ani hacer, aeke
            comer, ati beber, dunku dormir, aita saber, butu tomar, uni
            agua, hime pez, ada árbol, badia y, ika pero, bena cuando, kia
            después, kibe muy… Dos hermanas = reconstruida; una =
            hipotética (sólo llega por plantilla con el perfil era2). Donde
            el atestiguado ya lo decía, manda él (jusual, kidi, ateri, iero,
            ako, dare). Las viejas, archivadas con su capa
...
Número:     -kana (plural colectivo, reconstruido desde el wayunaiki — la
            afirmación «cognado directo con caquetío» bajó en d21.8)
Derivación: un verbo es también su nombre, sin marca — `jusual` «sembrar,
            siembra, sembradío» (Zavala #180); es lo que el atestiguado
            registra, no una afirmación sobre la lengua (dc.3 B)
...
del corpus se guarda sin lengua** (pasó: 27.641 de 54.936 usos). Y desde el
2026-09-20 la columna tiene dos valores que **no son lenguas**: `desconocida`
(raíz fuera del lexicón) y `acuñada` (lo mismo, pero la respuesta la declaró
como palabra nueva). Quien lea la columna buscando caquetío ya no los recoge —
que es el punto.
