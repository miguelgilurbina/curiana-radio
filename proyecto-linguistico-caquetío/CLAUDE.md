...
| **El mundo nombra voces del lexicón** | `6-fusion/sitios_era2.yaml` y `clima_era2.yaml` dicen la voz caquetía de cada cosa con `voz_caquetia: {clave, capa}`, y `[Tu tierra]` la enseña («Lo decís duna.»). Archivar o re-etiquetar una voz obliga a repuntarla ahí —y en los `palabra_lexicon` del corpus—: la tanda de las hermanas lo encontró por el pre-vuelo (14-20 de 126 prompts), y `6-fusion/scripts/verificar_sitios_era2.py` daba 22 fallos sin que ningún guardián lo viera. Desde ese día es el **décimo guardián** |
