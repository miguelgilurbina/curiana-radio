...
| **El hispanismo de origen indígena NO es un préstamo de esfera** | «yuca», «casabe», «maíz» son clave del lexicón (taíno) **y** palabra corriente del castellano: cuando el agente las escribe está escribiendo castellano, no tomando prestado de las islas. El día 1 de la serie B (run `3973d317`) `loanword_uses` guardó 7 usos y 6 eran esto; en TODA la base, 111 de las 161 formas-respuesta con préstamo (69%) son una de cinco voces. Desde el 2026-09-17 hay `HISPANISMOS_DE_ESFERA` (13 voces) y salen a un tercer cubo, ni préstamo ni fuga. **No mueve el score** —a propósito: su sitio natural es `CASTELLANO_CORRIENTE`, pero eso les quitaría la densidad arahuaca y movería 147 scores a mitad de serie; pasarlas allí es decisión del CORTE DE SERIE. El criterio es la ortografía de la clave: `maisi` («→ español maíz») sigue siendo préstamo, `maíz` no; `watapana` tampoco, el castellano dice dividivi. Sólo el token PELADO: `ta-casabe` sigue siendo préstamo. Los runs ya corridos se leen con `analizar_runs.py --prestamos`, que filtra al LEER (la base no se reescribe) y lo dice; `--con-hispanismos` los devuelve. Medido en `6-fusion/medicion_hispanismos_loanword_uses_2026-09-17.yaml`, que además deja escritas dos cosas para Miguel: el bloque `[Voces de fuera]` enseña 9 palabras castellanas, y las voces caquetías de maíz y yuca (`marisi`, `kasiripa`) son hipotéticas — el perfil `era2` las esconde, así que el agente no tenía lo propio delante |
