---
tipo: fuente
obra: "Parque metropolitano Laguna de Guaranao — entrada enciclopédica"
autor: "Wikipedia en español (colaborativa)"
anio: "2026"
publicacion: "Wikipedia es, «Parque metropolitano Laguna de Guaranao», consultada el 2026-09-16; cita cuatro referencias (blogs, archivos web y un documento legal venezolano)"
genero: datos
local: ""
paginas: "— (página web)"
acceso: "Libre en línea. https://es.wikipedia.org/wiki/Parque_metropolitano_Laguna_de_Guaranao"
capa_texto: web
estado_minado: minado
prioridad: media
sostiene: {hechos_corpus: 0, entradas_lexicon: 0}
verificado: 2026-09-16
minado: 2026-09-16
aliases: ["Laguna de Guaranao", "Parque metropolitano Laguna de Guaranao", "laguna-guaranao-parque"]
---

# Parque metropolitano Laguna de Guaranao — la laguna del nodo GUARANAO, hoy

> **Qué es.** Una **capa de datos moderna** (regla 3) sobre la laguna que da
> nombre al nodo GUARANAO de la era 2. Describe el medio de hoy —manglar,
> fauna, la salina que fue— y no dice nada de la gente del s. XV. Se abrió el
> 2026-09-16 por decisión de Miguel («Sí»), para que sus cuatro testimonios
> sobre la laguna («manglares, yacarés y peces»; la salina) tuvieran fuente:
> hasta entonces eran `testimonio-miguel` con `deuda: sin-procedencia` en
> `6-fusion/sitios_era2.yaml`.

## Qué se le preguntó

¿Hay manglar y caimán en la laguna de Guaranao, y fue salina?

## Qué da

| Dato | Texto | Etiqueta |
|---|---|---|
| **Manglar, cuatro especies** | «mangle rojo (*Rhizophora mangle*), el mangle negro (*Avicennia germinans*), el mangle llorón (*Laguncularia racemosa*) y botoncillo (*Conocarpus erectus*)»; alrededor «acacias, cujíes y guayacán», tunas, cardones | atestiguado (moderno) — **post-1985, no proyectable** (ver corrección) |
| **Los dos caimanes** | «la baba o el caimán de anteojos (*Caiman crocodilus*)» y «cocodrilos de la costa (*Crocodylus acutus*)» | **hipotético**: sólo esta página lo dice; GBIF/iNaturalist dan 0 registros de ambas especies en Paraguaná (2026-09-16) y el Libro Rojo 2015 da *C. acutus* en Falcón oriental sin nombrar la península |
| Peces y demás fauna | lisa, corocoro, lenguado; tortugas, serpientes, aves, tarántulas | atestiguado (moderno) |
| **La salina** | «En la década de 1970 era una salina utilizada antiguamente por los indios caquetíos» | atestiguado (moderno) como dato del s. XX; **la atribución a los caquetíos es afirmación moderna sobre el pasado** (regla 3) — la salina precontacto la sostiene [[arcaya-1920]], p. 22 |
| De salina a laguna salobre | «se descubrieron manantiales de agua dulce que la convirtieron en un ambiente salobre» | atestiguado (moderno) |
| La contaminación | «para 1980, a raíz del crecimiento de la ciudad de Punto Fijo, sufre de contaminación por aguas servidas» | atestiguado (moderno) |
| Dónde | Punto Fijo, Falcón; el mapa vivo (OSM `paraguana-1.json`) la registra como `Laguna de Guaranao`, `water=lake`, 11.6825, −70.1959 | atestiguado |

## Lo que vale para el proyecto

> ⚠️ **Corrección, el mismo 2026-09-16 (regla 2: en duda, degradar).** El
> rastreo web del cierre del día 1 halló la fecha del ecosistema: Aular Leal
> 2014 (*El Falconiano*, con la ing. forestal Keysa Sánchez de González, 33
> años en el Ministerio del Ambiente; ficha propuesta en
> `6-fusion/fichas_propuestas/aular-leal-2014-guaranao.md`) — «En su
> desembocadura inicialmente hubo una salina…»; «Fue en 1985 cuando el
> colector de aguas negras que pasa al margen norte de la quebrada se partió
> en un sector… hasta hacerlo la laguna que existe hoy»; «Creció un frondoso
> manglar». Esta misma página lo insinuaba (manantiales que la volvieron
> salobre; aguas servidas hacia 1980). **La laguna y el manglar tienen
> cuarenta años**: describen el sitio que Miguel vio, no el del s. XV. Para
> la era 2, Guaranao es **sal y ensenada** (la salina en la boca de una
> quebrada, [[arcaya-1920]] p. 22). El caimán baja a hipotético: 0 registros
> en GBIF/iNaturalist para la península y el Libro Rojo no la nombra.

- **Los testimonios de Miguel tenían razón sobre el sitio de hoy, y ahora
  tienen fuente y fecha.** Manglar, caimán y peces son `atestiguado (moderno)`
  como descripción del presente y **no se proyectan**; la salina sí, por
  Arcaya. El «yararé» era un lapsus por *yacaré*: por eso daba cero en todas
  las fuentes.
- **La salina corrobora a Arcaya**, que da las salinas de Guaranao como «las
  más importantes» de Paraguaná (p. 22). Es la segunda fuente, independiente,
  y moderna: vale como constante del medio (ecologia-002), no como dato de
  uso precontacto.
- **El caimán en el mundo, la palabra en el hueco.** El animal entra al
  canon de sitios con fuente; la voz caquetía sigue sin existir (`yacare` es
  'pueblo' en el canon de topónimos, Zavala #279; `caiman` es taíno y llega
  al tier 1 por la esfera de contacto — decisión del 2026-09-15).
- **Regla 6, al revés.** El mapa vivo no etiqueta humedal alguno a menos de
  6 km de la laguna: era un cero de cobertura, no de la laguna.

## Qué no da

- Ni la superficie del parque ni su fecha de creación.
- Nada precontacto: «utilizada por los indios caquetíos» no trae fuente
  propia.
- Referencias flojas (blogs, archivos web, un documento legal): es la misma
  calidad que la ficha del cerro; si aparece un inventario de INPARQUES o de
  la Alcaldía, sustituye a esta.

## Deuda

- Un inventario de fauna y flora de la laguna con autoría (Alcaldía de
  Carirubana, INPARQUES, MINEC) que reemplace a Wikipedia como fuente.
- El caimán de la costa en Paraguaná con fuente científica (el trabajo
  «Situación actual del caimán de la costa en Venezuela» devolvió 403).

## Enlaces

[[arcaya-1920]] · [[monumento-cerro-santa-ana]] · [[esteves-1989]] · `6-fusion/sitios_era2.yaml` · `6-fusion/decisiones_tanda_2026-09-15.yaml` (p3, p8)
